...
<body>
  ...
  <div id="settings">
    ...
    <p class="hint">
      ...
      Decrease X trim if it overshoots.
    </p>
  </div>
...
(() => {
  ...
  function loadCfg() {
    try {
      return {...defaults, ...(JSON.parse(localStorage.getItem('pc_touch_mouse_v4') || '{}'))};
    } catch {
      return {...defaults};
    }
  }
  function saveCfg() {
    localStorage.setItem('pc_touch_mouse_v4', JSON.stringify(cfg));
  }
  function prettyMode() {
  ...
  for (const id of ['speed','xTrim','yTrim','tapMove','longPress']) {
    els[id].oninput = () => {
...
def main():
    ...
    print()
    print("Pixel Challenge Phone Touch Mouse v4")
    print("------------------------------------")
    ...
    print()
    print("The Settings button is now always visible in the top bar.")
