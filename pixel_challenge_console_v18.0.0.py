# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v18.0.0
- Console owns controller + full button mapping; games consume it.
- Mapping saved in /home/ledgame/easter_game/controller_assignments.json
- White button is multi-use: Check-in only during OPEN state; acts as normal button during GAME.
- Safe play_sound / push_viewer_state (no crashes)
- Dot Dash: Uses stored Red/Blue mapping (no per-round learning)
"""
import os
import json
import time
...
class PixelChallengeConsole:
    def __init__(self, root: tk.Tk):
        ...
        self.white_button_index = 4  # Default fallback, overwritten by mapping
    ...
    def on_start_game(self):
        ...
        # We need to construct PlayerConfigs using the Mapped Buttons
        # For each checked-in player, we look up their "Red" and "Blue" button indices
        # But wait, GameManager/DotDash expects strings like "P1_A" or similar.
        # Actually, in this v18 architecture, we will simply pass "P{n}_RED" and "P{n}_BLUE" 
        # as the button identifiers. The Console's button poller will translate raw clicks
        # into these semantic events.
        ...
        for idx in range(1, 5):
            if self.player_status[idx]["checked_in"] and self.player_status[idx]["state"] != "REMOVED":
                ...
                # By convention for v18:
                # Button A -> P{idx}_RED
                # Button B -> P{idx}_BLUE
                # This way DotDash doesn't need to learn anything. It just uses A and B.
                p_cfg = PlayerConfig(
                    player_id=idx,
                    name=f"Player {idx}",
    ...
    def refresh_controller_panel(self):
        ...
        # Reassign button now doubles as "MAP BUTTONS" toggle
    ...
    def handle_map_input(self, player_id, button_index):
        # We are mapping for map_current_controller. If input matches, save it.
        if player_id != self.map_current_controller:
            return # Ignore other players while mapping Px
        
        color_name = BUTTON_MAP_ORDER[self.map_current_button_idx]
        
        # Save to assignment_map
        p_str = str(player_id)
        if p_str not in self.assignment_map:
            # Should exist if ONLINE, but safety check
    ...
    def apply_assignments(self):
        # Match discovered devices to saved assignments by signature
        self.joystick_player_map = {}
        ...
        for p_str, data in self.assignment_map.items():
            ...
            for dev in self.discovered_devices:
                if dev["signature"] == saved_sig:
                    ...
                    assigned_sigs.add(saved_sig)
                    found = True
                    break
            
            if not found:
                pass # remains MISSING

        # Basic fallback assignment for unassigned controllers to unassigned slots (optional)
        # For simplicity in v18, we rely on MAP BUTTONS/SCAN to handle identity or just edit JSON if desperate.
        # But let's auto-assign pure new controllers to empty slots if no collision?
        # (Skipping for now to keep logic strict: you must assign/map reliably)
        ...
        try:
            ...
            for js_index, js in self.joysticks.items():
                ...
                if not player_num and not self.button_map_mode: 
                    continue # Ignore unbound controllers unless mapping? 
                    # Actually mapping usually iterates strictly.
                
                # If mapping mode, we might need to know which JS corresponds to current mapping target
                # We assume the user has already SCANned so `joystick_player_map` is valid.
                ...
                for button_index in range(button_count):
                    ...
                    if pressed and not last_pressed:
                        if self.button_map_mode:
                            # If this joystick belongs to the player currently being mapped
    ...
    def handle_gameplay_button(self, player_id, button_index):
        # 1. Translate raw button_index to logical name via map
        ...
        # 2. White Button Handling
        # If it's mapped as white (or fallback if unmapped?)
        ...
        # 3. Game Inputs (Red, Green, Blue, etc)
        if self.host_state == HostState.GAME_RUNNING and logical_name:
            # Send event like "P1_RED", "P2_BLUE"
