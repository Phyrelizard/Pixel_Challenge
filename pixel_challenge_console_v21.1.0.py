...
class PixelChallengeConsole:
    def __init__(self, root: tk.Tk):
        ...
        self.sash_bottom_log = None  # NEW: for bottom panel left adjustment
...
"""
    ...
    def build_bottom_area(self, parent):
        ...
        # Use PanedWindow for adjustable left side
        ...
        left_buttons_frame = tk.Frame(self.bottom_paned, bg="#12061f")
        left_buttons_frame.grid_rowconfigure(0, weight=1)
        left_buttons_frame.grid_columnconfigure(0, weight=1)
        
        btn_container = tk.Frame(left_buttons_frame, bg="#12061f")
        btn_container.pack(fill="both", expand=True, padx=8, pady=8)
        
        # SETUP button (restored!)
        self.neon_button(btn_container, "SETUP", self.open_setup_window, bg="#1b63ff", width=14).pack(fill="x", pady=4)
        self.neon_button(btn_container, "FALCON CONSOLE", self.toggle_falcon_console, bg="#1b63ff", width=14).pack(fill="x", pady=4)
        self.neon_button(btn_container, "REDEEM POINTS", self.on_redeem_points, bg="#d48a10", width=14).pack(fill="x", pady=4)
        
        # Version label
        tk.Label(btn_container, text=VERSION_LABEL, bg="#12061f", fg="#9a9a9a", font=("Arial", 12, "bold")).pack(side="bottom", anchor="w", pady=(10, 0))
        
        self.bottom_paned.add(left_buttons_frame, minsize=180)
    ...
    # =========================================================================
    # SETUP WINDOW (FULL VERSION RESTORED)
    # =========================================================================
    def open_setup_window(self):
        ...
        self.setup_window.geometry(self.setup_geometry if self.setup_geometry else "800x600+2100+120")
        ...
        tk.Label(self.setup_window, text="SYSTEM SETUP", bg="#0f0617", fg="white", font=("Arial", 24, "bold")).pack(pady=(12, 16))

        # Create scrollable frame for settings
        canvas = tk.Canvas(self.setup_window, bg="#0f0617", highlightthickness=0)
        scrollbar = tk.Scrollbar(self.setup_window, orient="vertical", command=canvas.yview)
        ...
        falcon_frame = tk.LabelFrame(scrollable_frame, text="Falcon Controller", bg="#0f0617", fg="white", font=("Arial", 14, "bold"))
        falcon_frame.pack(fill="x", padx=20, pady=8)
        
        falcon_row = tk.Frame(falcon_frame, bg="#0f0617")
        falcon_row.pack(fill="x", padx=10, pady=8)
        tk.Label(falcon_row, text="Falcon IP:", bg="#0f0617", fg="white", font=("Arial", 12)).pack(side="left", padx=4)
        self.falcon_ip_entry = tk.Entry(falcon_row, font=("Arial", 12), width=18)
        self.falcon_ip_entry.insert(0, self.falcon_ip)
        self.falcon_ip_entry.pack(side="left", padx=8)
        self.neon_button(falcon_row, "TEST", lambda: self.test_falcon(self.falcon_ip_entry.get()), bg="#2ea62e", width=6).pack(side="left", padx=4)

        # --- WiFi Settings Section ---
        wifi_frame = tk.LabelFrame(scrollable_frame, text="WiFi Settings", bg="#0f0617", fg="white", font=("Arial", 14, "bold"))
        wifi_frame.pack(fill="x", padx=20, pady=8)
        
        wifi_grid = tk.Frame(wifi_frame, bg="#0f0617")
        wifi_grid.pack(fill="x", padx=10, pady=8)
        
        tk.Label(wifi_grid, text="SSID:", bg="#0f0617", fg="white", font=("Arial", 12)).grid(row=0, column=0, sticky="e", padx=4, pady=4)
        tk.Entry(wifi_grid, textvariable=self.wifi_ssid, font=("Arial", 12), width=24).grid(row=0, column=1, padx=4, pady=4)
        
        tk.Label(wifi_grid, text="Password:", bg="#0f0617", fg="white", font=("Arial", 12)).grid(row=1, column=0, sticky="e", padx=4, pady=4)
        tk.Entry(wifi_grid, textvariable=self.wifi_psk, font=("Arial", 12), width=24, show="*").grid(row=1, column=1, padx=4, pady=4)
        
        tk.Label(wifi_grid, text="Static IP:", bg="#0f0617", fg="white", font=("Arial", 12)).grid(row=2, column=0, sticky="e", padx=4, pady=4)
        tk.Entry(wifi_grid, textvariable=self.wifi_static_ip, font=("Arial", 12), width=24).grid(row=2, column=1, padx=4, pady=4)
        
        tk.Label(wifi_grid, text="Gateway:", bg="#0f0617", fg="white", font=("Arial", 12)).grid(row=3, column=0, sticky="e", padx=4, pady=4)
        tk.Entry(wifi_grid, textvariable=self.wifi_gateway, font=("Arial", 12), width=24).grid(row=3, column=1, padx=4, pady=4)

        # --- Ethernet Settings Section ---
        eth_frame = tk.LabelFrame(scrollable_frame, text="Ethernet Settings", bg="#0f0617", fg="white", font=("Arial", 14, "bold"))
        eth_frame.pack(fill="x", padx=20, pady=8)
        
        eth_grid = tk.Frame(eth_frame, bg="#0f0617")
        eth_grid.pack(fill="x", padx=10, pady=8)
        
        tk.Label(eth_grid, text="Static IP:", bg="#0f0617", fg="white", font=("Arial", 12)).grid(row=0, column=0, sticky="e", padx=4, pady=4)
        tk.Entry(eth_grid, textvariable=self.eth_static_ip, font=("Arial", 12), width=24).grid(row=0, column=1, padx=4, pady=4)
        
        tk.Label(eth_grid, text="Gateway:", bg="#0f0617", fg="white", font=("Arial", 12)).grid(row=1, column=0, sticky="e", padx=4, pady=4)
        tk.Entry(eth_grid, textvariable=self.eth_gateway, font=("Arial", 12), width=24).grid(row=1, column=1, padx=4, pady=4)

        # --- DNS/NTP Settings Section ---
        dns_frame = tk.LabelFrame(scrollable_frame, text="DNS / NTP Settings", bg="#0f0617", fg="white", font=("Arial", 14, "bold"))
        dns_frame.pack(fill="x", padx=20, pady=8)
        
        dns_grid = tk.Frame(dns_frame, bg="#0f0617")
        dns_grid.pack(fill="x", padx=10, pady=8)
        
        tk.Label(dns_grid, text="DNS Server:", bg="#0f0617", fg="white", font=("Arial", 12)).grid(row=0, column=0, sticky="e", padx=4, pady=4)
        tk.Entry(dns_grid, textvariable=self.dns_server, font=("Arial", 12), width=24).grid(row=0, column=1, padx=4, pady=4)
        
        tk.Label(dns_grid, text="NTP Server:", bg="#0f0617", fg="white", font=("Arial", 12)).grid(row=1, column=0, sticky="e", padx=4, pady=4)
        tk.Entry(dns_grid, textvariable=self.ntp_server, font=("Arial", 12), width=24).grid(row=1, column=1, padx=4, pady=4)

        # --- System Settings Section ---
        sys_frame = tk.LabelFrame(scrollable_frame, text="System Settings", bg="#0f0617", fg="white", font=("Arial", 14, "bold"))
        sys_frame.pack(fill="x", padx=20, pady=8)
        
        sys_grid = tk.Frame(sys_frame, bg="#0f0617")
        sys_grid.pack(fill="x", padx=10, pady=8)
        
        tk.Label(sys_grid, text="Hostname:", bg="#0f0617", fg="white", font=("Arial", 12)).grid(row=0, column=0, sticky="e", padx=4, pady=4)
        tk.Entry(sys_grid, textvariable=self.hostname, font=("Arial", 12), width=24).grid(row=0, column=1, padx=4, pady=4)
        
        tk.Checkbutton(sys_grid, text="Auto-start on boot", variable=self.auto_start, bg="#0f0617", fg="white", activebackground="#0f0617", activeforeground="white", selectcolor="#17071f", font=("Arial", 12)).grid(row=1, column=0, columnspan=2, sticky="w", padx=4, pady=4)
        ...
        btn_row.pack(fill="x", pady=(16, 12))
        self.neon_button(btn_row, "SAVE & APPLY", self.save_setup, bg="#2ea62e", width=14).pack(side="left", padx=20)
        self.neon_button(btn_row, "CANCEL", self.close_setup_window, bg="#c93b1e", width=10).pack(side="left", padx=10)
        self.neon_button(btn_row, "REBOOT SYSTEM", self.reboot_system, bg="#ff6600", width=14).pack(side="right", padx=20)
