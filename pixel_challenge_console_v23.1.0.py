# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v23.1.0
...
"""
    ...
    def build_ui(self):
        ...
        # RIGHT SIDE: vertical split — content on top, log below, buttons at bottom (v23.0.0)
        ...
        # Vertical paned window: content area (top) | log (bottom)
        ...
        # Content paned window: DMX+AUDIO | PLAYER | CONTROLLERS (horizontal)
        ...
        # Pane 1: DMX CONTROL + AUDIO MIXER stacked vertically
        self.dmx_audio_container = tk.Frame(self.content_paned, bg="#12061f")
        self.content_paned.add(self.dmx_audio_container, minsize=200)

        # Pane 2: PLAYER STATUS (check-in + 2×2 grid)
        self.center_container = tk.Frame(self.content_paned, bg="#12061f")
        self.content_paned.add(self.center_container, minsize=400)

        # Pane 3: CONTROLLERS (unchanged 2×2 grid)
        ...
        # Log panel (spans full width below the content panes)
        log_frame = tk.Frame(self.main_vertical, bg="#12061f")
        self.main_vertical.add(log_frame, minsize=MIN_INFO_HEIGHT)
        ...
        self.build_dmx_audio_area(self.dmx_audio_container)
        self.build_controllers_area(self.controllers_container)
        self.build_log_area(log_frame)
    ...
    def restore_sashes(self):
        ...
        # Main vertical (content | log split) — reusing sash_bottom_log key
        try:
        ...
        # Content paned sash 0: DMX+AUDIO | PLAYER — reusing sash_center_mixer key
        try:
            if self.sash_center_mixer and hasattr(self, 'content_paned'):
                ...
                self.content_paned.sash_place(0, 220, 0)
        except Exception:
            pass

        # Content paned sash 1: PLAYER | CONTROLLERS — reusing sash_center_ctrl key
        try:
            if self.sash_center_ctrl and hasattr(self, 'content_paned'):
                self.content_paned.sash_place(1, int(self.sash_center_ctrl), 0)
            elif hasattr(self, 'content_paned'):
                self.content_paned.sash_place(1, max(620, total_w - MIN_CONTROLLERS - 100), 0)
        except Exception:
            pass
    ...
    def build_dmx_audio_area(self, parent):
        ...
        dmx_panel.grid(row=0, column=0, sticky="ew", pady=(0, 4))

        # 8 vertical sliders side-by-side
        faders_frame = tk.Frame(dmx_body, bg="#17071f")
        faders_frame.pack(fill="x", pady=(4, 2))
        for i in range(8):
            col_frame = tk.Frame(faders_frame, bg="#17071f")
            col_frame.pack(side="left", fill="y", padx=2, expand=True)
            tk.Scale(col_frame, from_=255, to=0, resolution=1, orient="vertical",
                     variable=self.dmx_channels[i],
                     bg="#17071f", fg="white", troughcolor="#071a30",
                     highlightthickness=0, font=("Arial", 7, "bold"),
                     length=150, tickinterval=85,
                     width=14).pack()
            tk.Label(col_frame, text=str(i + 1), bg="#17071f", fg="#cccccc",
                     font=("Arial", 9, "bold")).pack()

        # BLACKOUT and AUTO CYCLE buttons (placeholder DMX controls)
        dmx_btn_frame = tk.Frame(dmx_body, bg="#17071f")
        dmx_btn_frame.pack(fill="x", pady=(4, 4))
        tk.Button(dmx_btn_frame, text="BLACKOUT",
                  command=lambda: self.log("DMX Blackout (not implemented)"),
                  bg="#c93b1e", fg="white", activebackground="#c93b1e", activeforeground="white",
                  relief="raised", bd=2, font=("Arial", 11, "bold"),
                  padx=4, pady=4, cursor="hand2").pack(side="left", fill="x", expand=True, padx=(0, 2))
        tk.Button(dmx_btn_frame, text="AUTO CYCLE",
                  command=lambda: self.log("DMX Auto Cycle (not implemented)"),
                  bg="#2ea62e", fg="white", activebackground="#2ea62e", activeforeground="white",
                  relief="raised", bd=2, font=("Arial", 11, "bold"),
                  padx=4, pady=4, cursor="hand2").pack(side="left", fill="x", expand=True, padx=(2, 0))

        # --- AUDIO MIXER panel (below DMX) ---
