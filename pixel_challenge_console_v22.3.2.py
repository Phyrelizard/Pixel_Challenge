# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v22.3.2
...
"""
    ...
    def open_config_window(self):
        ...
        try:
            ...
            self.log(f"Opening config: {config_path}")
            
            # Check if config file exists
            if not os.path.exists(config_path):
                messagebox.showwarning("Config Not Found", f"Config file not found:\n{config_path}")
                return
            ...
            self.config_window.title(f"Config Editor - {self.selected_game.get()}")
            
            # Safe geometry handling
            try:
                geom = self.config_geometry if self.config_geometry else "800x600"
                self.config_window.geometry(geom)
            except Exception:
                self.config_window.geometry("800x600")
            
            self.config_window.configure(bg="#12061f")
            self.config_window.transient(self.root)
            
            # Header with SAVE and CLOSE buttons
            header = tk.Frame(self.config_window, bg="#0f0617")
            header.pack(fill="x", padx=5, pady=5)
            
            tk.Button(
                header, text="SAVE", command=self.save_config_file,
                bg="#2ea62e", fg="white", font=("Arial", 12, "bold"),
                width=8, cursor="hand2"
            ).pack(side="left", padx=5)
            
            tk.Label(
                header, text=f"Editing: {os.path.basename(config_path)}",
                bg="#0f0617", fg="white", font=("Arial", 12, "bold")
            ).pack(side="left", padx=20)
            
            tk.Button(
                header, text="CLOSE", command=self.close_config_window,
                bg="#c93b1e", fg="white", font=("Arial", 12, "bold"),
                width=8, cursor="hand2"
            ).pack(side="right", padx=5)
            
            # Text editor
            text_frame = tk.Frame(self.config_window, bg="#12061f")
            ...
            self.config_text = tk.Text(
                text_frame, wrap="none", font=("Courier", 11),
                bg="#1a1a2e", fg="#00ff00", insertbackground="white",
            ...
            self.config_window.config_path = config_path
            
            # Handle window close
            self.config_window.protocol("WM_DELETE_WINDOW", self.close_config_window)
            
            self.log(f"Config window opened for: {os.path.basename(config_path)}")
            
        except Exception as e:
            self.log(f"ERROR opening config window: {e}")
            import traceback
            self.log(traceback.format_exc())
