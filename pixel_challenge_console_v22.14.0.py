# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v22.14.0
...
"""
    ...
    def play_sound(self, sound_key: str):
        ...
        try:
            ...
            elif "voice" in sound_key:
                # Voice prompts use their own volume slider (v22.14.0)
    ...
    def build_bottom_area(self, parent):
       ...
       # Left side - Audio Mixer panel
        ...
        music_row.pack(fill="x", pady=(6, 2))
        tk.Label(music_row, text="MUSIC VOLUME (%)", bg="#17071f", fg="#cccccc", font=("Arial", 14, "bold")).pack(side="left", padx=(0, 8))
        self.music_mute_btn = tk.Button(music_row, text="MUSIC", command=self.toggle_music_mute,
                                         ...
                                         relief="raised", bd=2, font=("Arial", 11, "bold"), padx=8, pady=2, cursor="hand2")
        ...
        sfx_row.pack(fill="x", pady=(4, 2))
        tk.Label(sfx_row, text="SFX VOLUME (%)", bg="#17071f", fg="#cccccc", font=("Arial", 14, "bold")).pack(side="left", padx=(0, 8))
        self.sfx_mute_btn = tk.Button(sfx_row, text="SFX", command=self.toggle_sfx_mute,
                                       bg="#27a844", fg="white", activebackground="#27a844", activeforeground="white",
                                       relief="raised", bd=2, font=("Arial", 11, "bold"), padx=8, pady=2, cursor="hand2")
        self.sfx_mute_btn.pack(side="right")
        tk.Scale(mixer_body, from_=0, to=100, resolution=1, orient="horizontal", variable=self.sfx_volume, bg="#17071f", fg="white", troughcolor="#071a30", highlightthickness=0, font=("Arial", 12, "bold"), command=self.on_sfx_volume_changed).pack(fill="x", pady=(0, 4))

        # --- Voice Volume row with mute button (v22.14.0) ---
        voice_row = tk.Frame(mixer_body, bg="#17071f")
        voice_row.pack(fill="x", pady=(4, 2))
        tk.Label(voice_row, text="VOICE VOLUME (%)", bg="#17071f", fg="#cccccc", font=("Arial", 14, "bold")).pack(side="left", padx=(0, 8))
        self.voice_mute_btn = tk.Button(voice_row, text="VOICE", command=self.toggle_voice_mute,
                                         bg="#27a844", fg="white", activebackground="#27a844", activeforeground="white",
                                         relief="raised", bd=2, font=("Arial", 11, "bold"), padx=8, pady=2, cursor="hand2")
        self.voice_mute_btn.pack(side="right")
        tk.Scale(mixer_body, from_=0, to=100, resolution=1, orient="horizontal", variable=self.voice_volume, bg="#17071f", fg="white", troughcolor="#071a30", highlightthickness=0, font=("Arial", 12, "bold"), command=self.on_voice_volume_changed).pack(fill="x", pady=(0, 4))
