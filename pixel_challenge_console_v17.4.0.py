# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v17.4.0
- Adds CONFIG button (between GAME dropdown and VIEW SCOREBOARD) to view/edit per-game config JSON.
- Adds file logging to /home/ledgame/easter_game/log_YYYYMMDD.txt.
- Dot Dash mapping is fully parallel: any player can press any two gameplay buttons (not white) to map A/B; no turn order.
- Keeps all features from v17.3.0 (Falcon test, split theme/gameplay brightness, Falcon console button, etc.).
"""
import os
import json
import time
...
class PixelChallengeConsole:
    ...
    def open_config_window(self):
        ...
        if not os.path.exists(path):
            default_payload = {
                ...
                "sound_pack": "default",
                "notes": "auto-created by console v17.4.0; adjust as needed",
            }
    ...
    def begin_dot_dash_button_learning(self, player_id: int = 1):
        ...
        self.dot_dash_selected_colors[player_id] = {"color_a": None, "color_b": None, "color_a_name": None, "color_b_name": None}
    ...
    def handle_dot_dash_learning_press(self, player_num: int, button_index: int) -> bool:
        ...
        if self.dot_dash_learning_stage == 1:
            ...
            self.log(f"Dot Dash: Player {player_num} first button learned as raw {button_index}. Now press the SECOND game button.")
            return True
        if self.dot_dash_learning_stage == 2:
            ...
            self.log(f"Dot Dash: buttons learned. A=raw {mapping['button_a_index']}, B=raw {mapping['button_b_index']}.")
            self.log(f"Dot Dash: choose FIRST color. Press button A to cycle. Press button B to confirm. Current: {color_name}.")
            return True
        if self.dot_dash_learning_stage == 3:
            ...
            if button_index == mapping["button_b_index"]:
                ...
                self.log(
                    ...
                    f"Choose SECOND color. Press button A to cycle. Press button B to confirm. Current: {next_name}."
                )
                return True
        if self.dot_dash_learning_stage == 4:
            ...
            if button_index == mapping["button_b_index"]:
                ...
                self.dot_dash_learning_active = False
                self._render_dot_dash_setup_preview(player_num)
                self.log(f"Dot Dash: second color confirmed -> {color_name}. Using {selected['color_a_name']} / {selected['color_b_name']}.")
                self.start_dot_dash_module(player_num, selected["color_a"], selected["color_b"])
                return True
        return False
    ...
    def _handle_dot_dash_parallel_map(self, player_num: int, button_index: int):
        ...
        if mapping["button_a_index"] is None:
            ...
            self.host_api.play_sound("button_learned")
            return
