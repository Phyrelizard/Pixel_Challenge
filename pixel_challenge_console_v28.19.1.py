# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v28.19.1
...
class FalconService:
    def __init__(self, falcon_ip: str, pixels_per_lane: int = 100, dmx_universe: int = None, playfield_inverted: bool = False,
        ...
        # Temporal output dithering preserves ultra-dim colors after the global
        # Falcon brightness scaler would otherwise truncate low RGB channels to 0.
        # This is especially important for Ascend background glow values such as
        # purple at 0.04 where red can disappear before blue.
    ...
    def _dither_scaled_channel(self, value: float, scale: float, universe: int, pixel_index: int, channel_index: int) -> int:
        """Scale one RGB channel and preserve sub-1.0 values with temporal dithering.

        Without this, very dim colors can lose one channel after the global
        Falcon/gameplay brightness scaler truncates to integer DMX values. For
        example, a dim purple background may become blue because red rounds to
        0 while blue rounds to 1. Dithering lets the channel blink on for the
        right fraction of frames so the eye sees the intended average color.
        """
        scaled = max(0.0, min(255.0, float(value) * float(scale)))
        whole = int(math.floor(scaled))
        frac = scaled - whole
        if frac <= 0.0 or whole >= 255:
            return clamp8(whole)

        cycle = max(2, int(getattr(self, "_output_dither_cycle", 32)))
        # Derive a stable phase from time, universe, pixel, and channel. This
        # spreads the tiny on/off pulses so the whole background does not wink
        # on the same frame, while still averaging correctly over time.
        frame = int(time.monotonic() * 120.0)
        phase = (frame + int(universe) * 7 + int(pixel_index) * 11 + int(channel_index) * 17) % cycle
        return clamp8(whole + (1 if phase < int(round(frac * cycle)) else 0))
