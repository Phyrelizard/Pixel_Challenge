# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v23.0.0
...
class PixelChallengeConsole:
    def __init__(self, root: tk.Tk):
        ...
        self._sound_cache = {}  # v23.0.0 – pre-loaded Sound objects keyed by sound_key
...
"""
    ...
    # v23.0.0 – sound-path mapping extracted so _preload_sounds can reuse it
    SOUND_PATHS = {
        # Pixel Pop sounds
    ...
    def _preload_sounds(self):
        """v23.0.0 – Pre-load all non-music Sound objects into _sound_cache
        so play_sound never has to read from disk during gameplay."""
        if not pygame.mixer.get_init():
            return
        loaded = 0
        for key, path in self.SOUND_PATHS.items():
            if "music" in key:
                continue  # music tracks are streamed, not cached
            if not os.path.exists(path):
                if self.debug_logging.get():
                    self.log(f"[AUDIO] Preload skip – file not found: {path}")
                continue
            try:
                self._sound_cache[key] = pygame.mixer.Sound(path)
                loaded += 1
            except Exception as e:
                if self.debug_logging.get():
                    self.log(f"[AUDIO] Preload error for {key}: {e}")
        self.log(f"[AUDIO] Pre-loaded {loaded} sound effects")

    def play_sound(self, sound_key: str):
        """Play a sound effect by key.  v23.0.0 – uses pre-loaded cache."""
        ...
        try:
            ...
            # Background music is streamed (not cached)
            ...
            # Use cached Sound object; fall back to loading from disk
            ...
            if "voice" in sound_key or "screen_" in sound_key:
    ...
    def build_ui(self):
        ...
        # RIGHT SIDE: horizontal split — left column (center+mixer) vs right column (controllers)
        # v22.16.0: center and mixer share a VERTICAL pane so the mixer sash
        #           only eats into the center area, never the controllers.
        ...
        # --- Left column of right side: center + bottom stacked vertically ---
        center_mixer_pane = tk.Frame(self.right_horizontal, bg="#12061f")
        self.right_horizontal.add(center_mixer_pane, minsize=MIN_CENTER)
        
        center_mixer_pane.grid_rowconfigure(0, weight=1)
        center_mixer_pane.grid_columnconfigure(0, weight=1)
        
        self.center_mixer_vertical = tk.PanedWindow(center_mixer_pane, orient="vertical", sashwidth=8, sashrelief="raised", bg="#0b0314", opaqueresize=True)
        self.center_mixer_vertical.pack(fill="both", expand=True)
        
        # Top: center area (check-in + player status)
        self.center_container = tk.Frame(self.center_mixer_vertical, bg="#12061f")
        self.center_mixer_vertical.add(self.center_container, minsize=120)
        
        # Bottom: mixer + log (side by side)
        self.bottom_container = tk.Frame(self.center_mixer_vertical, bg="#12061f")
        self.center_mixer_vertical.add(self.bottom_container, minsize=MIN_INFO_HEIGHT)
        
        # --- Right column of right side: controllers (completely independent) ---
        ...
        self.center_mixer_vertical.bind("<ButtonRelease-1>", self.save_sash_positions)
    ...
    def restore_sashes(self):
        ...
        # Center/mixer vertical sash (v22.16.0 — independent of controllers)
        try:
            if self.sash_center_mixer and hasattr(self, 'center_mixer_vertical'):
                self.center_mixer_vertical.sash_place(0, 0, int(self.sash_center_mixer))
            elif hasattr(self, 'center_mixer_vertical'):
                self.center_mixer_vertical.sash_place(0, 0, total_h - MIN_INFO_HEIGHT - 200)
        except Exception:
            pass
        
        # Right horizontal (center+mixer column vs controllers column)
        try:
        ...
        # Bottom mixer/log horizontal split
        try:
    ...
    def save_sash_positions(self, event=None):
        try:
            ...
            pass
        try:
            if hasattr(self, 'center_mixer_vertical'):
                self.sash_center_mixer = self.center_mixer_vertical.sash_coord(0)[1]
        except Exception:
            pass
        try:
    ...
    def build_center_area(self, parent):
        ...
        parent.grid_columnconfigure(0, weight=1)
        # v23.0.0 – PLAYER STATUS above buttons
        status_panel, status_body = self.panel(parent, "PLAYER STATUS")
        status_panel.grid(row=0, column=0, sticky="ew", pady=(0, 4))
        ...
        # v23.0.0 – CHECK-IN and CONFIRM on their own row (no JOINED between them)
        checkin_row = tk.Frame(enroll_body, bg="#17071f")
        checkin_row.pack(fill="x", pady=(4, 2))
        checkin_row.grid_columnconfigure(0, weight=1)
        ...
        self.checkin_button.grid(row=0, column=0, sticky="e", padx=(0, 6))
        self.confirm_button = self.neon_button(checkin_row, "CONFIRM", self.on_confirm_players, bg="#1b63ff", width=12)
        self.confirm_button.grid(row=0, column=1, sticky="w", padx=(6, 0))
        # v23.0.0 – JOINED under the buttons
        joined_row = tk.Frame(enroll_body, bg="#17071f")
        joined_row.pack(fill="x", pady=(2, 4))
        joined_inner = tk.Frame(joined_row, bg="#17071f")
        joined_inner.pack(anchor="center")
        tk.Label(joined_inner, text="JOINED:", bg="#17071f", fg="#ffd74f", font=("Arial", 16, "bold")).pack(side="left", padx=(0, 4))
        tk.Label(joined_inner, textvariable=self.players_joined, bg="#24101f", fg="#ffd74f", font=("Arial", 20, "bold"), width=2).pack(side="left")
    ...
    def build_bottom_area(self, parent):
        ...
        # Vertical paned window: mixer on top, log on bottom (v22.16.0)
        # This gives the mixer its own independent height sash
        self.bottom_vertical = tk.PanedWindow(parent, orient="vertical", sashwidth=8, sashrelief="raised", bg="#0b0314", opaqueresize=True)
        self.bottom_vertical.grid(row=0, column=0, sticky="nsew")

        # --- Audio Mixer panel (top) ---
        mixer_panel, mixer_body = self.panel(self.bottom_vertical, "AUDIO MIXER")

        # Music Volume row with mute button
        music_row = tk.Frame(mixer_body, bg="#17071f")
        music_row.pack(fill="x", pady=(4, 1))
        tk.Label(music_row, text="MUSIC (%)", bg="#17071f", fg="#cccccc", font=("Arial", 12, "bold")).pack(side="left", padx=(0, 4))
        self.music_mute_btn = tk.Button(music_row, text="MUSIC", command=self.toggle_music_mute,
                                         bg="#27a844", fg="white", activebackground="#27a844", activeforeground="white",
                                         relief="raised", bd=2, font=("Arial", 10, "bold"), padx=6, pady=1, cursor="hand2")
        self.music_mute_btn.pack(side="right")
        tk.Scale(mixer_body, from_=0, to=100, resolution=1, orient="horizontal", variable=self.music_volume,
                 bg="#17071f", fg="white", troughcolor="#071a30", highlightthickness=0,
                 font=("Arial", 10, "bold"), command=self.on_music_volume_changed).pack(fill="x", pady=(0, 2))

        # SFX Volume row with mute button
        sfx_row = tk.Frame(mixer_body, bg="#17071f")
        sfx_row.pack(fill="x", pady=(2, 1))
        tk.Label(sfx_row, text="SFX (%)", bg="#17071f", fg="#cccccc", font=("Arial", 12, "bold")).pack(side="left", padx=(0, 4))
        self.sfx_mute_btn = tk.Button(sfx_row, text="SFX", command=self.toggle_sfx_mute,
                                       bg="#27a844", fg="white", activebackground="#27a844", activeforeground="white",
                                       relief="raised", bd=2, font=("Arial", 10, "bold"), padx=6, pady=1, cursor="hand2")
        self.sfx_mute_btn.pack(side="right")
        tk.Scale(mixer_body, from_=0, to=100, resolution=1, orient="horizontal", variable=self.sfx_volume,
                 bg="#17071f", fg="white", troughcolor="#071a30", highlightthickness=0,
                 font=("Arial", 10, "bold"), command=self.on_sfx_volume_changed).pack(fill="x", pady=(0, 2))

        # Voice Volume row with mute button (v22.14.0)
        voice_row = tk.Frame(mixer_body, bg="#17071f")
        voice_row.pack(fill="x", pady=(2, 1))
        tk.Label(voice_row, text="VOICE (%)", bg="#17071f", fg="#cccccc", font=("Arial", 12, "bold")).pack(side="left", padx=(0, 4))
        self.voice_mute_btn = tk.Button(voice_row, text="VOICE", command=self.toggle_voice_mute,
                                         bg="#27a844", fg="white", activebackground="#27a844", activeforeground="white",
                                         relief="raised", bd=2, font=("Arial", 10, "bold"), padx=6, pady=1, cursor="hand2")
        self.voice_mute_btn.pack(side="right")
        tk.Scale(mixer_body, from_=0, to=100, resolution=1, orient="horizontal", variable=self.voice_volume,
                 bg="#17071f", fg="white", troughcolor="#071a30", highlightthickness=0,
                 font=("Arial", 10, "bold"), command=self.on_voice_volume_changed).pack(fill="x", pady=(0, 2))

        self.update_music_mute_button()
        self.update_sfx_mute_button()
        self.update_voice_mute_button()

        self.bottom_vertical.add(mixer_panel, minsize=80)

        # --- Log panel (bottom) ---
        info_panel = tk.Frame(self.bottom_vertical, bg="#3a1b53", bd=2, relief="groove")
        ...
        self.info_text = tk.Text(info_body, height=3, width=68, font=("Arial", 14), bg="#12061f", fg="white", wrap="word", bd=0, relief="flat")
        ...
        self.bottom_vertical.add(info_panel, minsize=60)

        # Bind sash movement to save
        self.bottom_vertical.bind("<ButtonRelease-1>", self.save_sash_positions)
