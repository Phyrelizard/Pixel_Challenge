# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v18.1.0
- FIX: Crash in push_viewer_state (accepts key + payload now).
- FIX: Crash in play_sound (accepts variable args).
- Console owns controller + full button mapping.
- White button is multi-use (Check-in vs Game).
"""
import os
import json
import time
...
class PixelChallengeConsole:
    ...
    def play_sound(self, *args):
        """Safe sound trigger; won’t crash if audio backend is absent or args mismatch."""
        try:
            # We assume first arg is sound_key
            if len(args) > 0:
                sound_key = args[0]
