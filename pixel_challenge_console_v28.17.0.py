# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v28.17.0
...
class SoundVisualizerEngine:
    ...
    def set_tuning(self, tuning: dict):
        ...
        if isinstance(tuning, dict):
            for key in ("direction", "input_mode", "stereo_mapping", "lab_mirror"):
                if key in tuning:
...
class PixelChallengeConsole:
    def __init__(self, root: tk.Tk):
        ...
        self.flame_theme_tuning = json.loads(json.dumps(DEFAULT_FLAME_TUNING))
        self.sound_visualizer_tuning = json.loads(json.dumps(DEFAULT_SOUND_VISUALIZER_TUNING))
...
"""
    ...
    def load_settings(self):
        ...
        try:
            ...
            self.sound_visualizer_tuning = self._normalize_sound_visualizer_tuning(data.get("sound_visualizer_tuning", {}))
    ...
    def open_sound_visualizer_tune_popup(self):
        ...
        tk.Label(win, textvariable=status_var, bg="#081827", fg="#cccccc",
                 font=("Arial", 10, "bold"), wraplength=560).pack(pady=(0, 8))
        ...
        def store_values(*_):
            self.sound_visualizer_tuning.update({
            ...
            for key, var in value_vars.items():
                self.sound_visualizer_tuning[key] = max(0, min(100, int(var.get())))
            self.sound_visualizer_tuning = self._normalize_sound_visualizer_tuning(self.sound_visualizer_tuning)
            self._push_sound_visualizer_tuning_to_falcon()
            self.save_settings()
        ...
        for row, (label, var, values) in enumerate(option_rows):
            ...
            combo = ttk.Combobox(body, textvariable=var, values=list(values), state="readonly",
                                 font=("Arial", 10, "bold"), width=34, justify="center")
        ...
        tk.Label(win, text="Tip: install sounddevice/PortAudio for live mic input; without it, this theme shows a test pulse.",
                 bg="#081827", fg="#b8dfff", font=("Arial", 9, "bold"), wraplength=560).pack(pady=(0, 8))

        def reset_values():
            self.sound_visualizer_tuning = json.loads(json.dumps(DEFAULT_SOUND_VISUALIZER_TUNING))
            direction_var.set(self.sound_visualizer_tuning["direction"])
            input_var.set(self.sound_visualizer_tuning["input_mode"])
            mapping_var.set(self.sound_visualizer_tuning["stereo_mapping"])
            mirror_var.set(self.sound_visualizer_tuning["lab_mirror"])
            for key in SOUND_VISUALIZER_NUMERIC_KEYS:
                value_vars[key].set(self.sound_visualizer_tuning[key])
                value_labels[key].configure(text=f"{value_vars[key].get():3d}%")
            store_values()
            self.log("Sound Visualizer tune reset")
