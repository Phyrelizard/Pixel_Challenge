# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v28.12.3
...
"""
    ...
    def _default_current_dmx_layout(self) -> dict:
        """Return the current 12-fixture rig layout used for v28.12.3.
    ...
    def _repair_known_visualizer_layout(self, data: dict) -> tuple[dict, bool]:
        """Repair old mixed-rig layouts that still point the dimmer pack at 37-40.

        The DMX runtime is intentionally layout-driven.  If the hardware setup
        screen says the DP-DMX4B starts at 65 but the layout still says F9-F12
        start at 37-40, the runtime will keep sending dimmer data to 37-40.
        This repair updates only the known F1-F12 rig entries and preserves any
        extra custom targets/fixtures the user added.
        """
        changed = False
        ...
        # Update/add the known current rig.  This is deliberately limited to
        # F1-F12 so any user-added fixtures beyond the core rig are left alone.
        for fid in [f"F{i}" for i in range(1, 13)]:
            ...
            for key in ("type", "profile_id", "universe", "start_address", "channels"):
                if got.get(key) != want.get(key):
                    ...
                    changed = True
            # Keep user-dragged x/y if present; fill in missing visual fields.
        ...
        if layout.get("name") != "Betopper + ThinTri + DP-DMX4B Rig":
            layout["name"] = "Betopper + ThinTri + DP-DMX4B Rig"
            changed = True
            ...
            if isinstance(data, dict) and isinstance(data.get("layouts"), list):
                ...
                if changed:
                    try:
                        with open(DMX_VISUALIZER_LAYOUTS_FILE, "w", encoding="utf-8") as f:
                            json.dump(data, f, indent=2)
                        self.log("DMX layout repaired: Betopper 1/9/17/25, ThinTri 33/41/49/57, dimmers 65-68.")
                    except Exception:
                        pass
                return data
        except Exception as e:
    ...
    def _infer_layout_fixture_profile_id(self, fixture: dict, profiles_by_id: dict) -> str:
        ...
        v28.12.3: do not classify every ``type: dimmer`` fixture as the
