# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v17.4.1
- File logging to /home/ledgame/easter_game/log_YYYYMMDD.txt
- CONFIG button/editor per game (and global for Splash)
- Dot Dash: fully parallel button mapping (no 1→4 turn order)
- Fix: play_sound stub so mapping no longer throws
- All features from v17.3.x retained (Falcon test, brightness split, etc.)
"""
import os
import json
import time
...
class FalconService:
    ...
    def _theme_pixels(self, theme_name: str, lane_slot: int, step: int):
        ...
        if theme_name == "lane chase lr":
            ...
            return [scale_color(base, 1.0)] * n if lane_slot == active_slot else [scale_color(base, 0.08)] * n
        ...
        if theme_name == "bounce chase":
            ...
            return [scale_color(base, 1.0)] * n if lane_slot == active_slot else [scale_color(base, 0.06)] * n
...
class PixelChallengeConsole:
    def __init__(self, root: tk.Tk):
        ...
        self.dot_dash_mapping_pending = False
        self.dot_dash_needed_players: set[int] = set()
    ...
    def play_sound(self, sound_key: str):
        """Stub to avoid crashes if host_api playback fails or is unavailable."""
        try:
    ...
    def show_scoreboard_temporarily(self, seconds: int = 10, payload=None, final=False):
        ...
        if final:
            ...
            self.log("ViewerService: show Final Results (placeholder).")
        else:
    ...
    def build_top_bar(self):
        ...
        tk.Checkbutton(btns, text="Show Ranking", variable=self.show_ranking, bg="#0f0617", fg="white", activebackground="#0f0617", activeforeground="white", selectcolor="#17071f", font=("Arial", 14, "bold"), highlightthickness=0, bd=0).pack(side="left", padx=(0, 8))
        ...
        save_btn = tk.Button(top, text="Save Layout", command=self.save_layout_now, bg="#9440ff", fg="white", activebackground="#9440ff", activeforeground="white", relief="raised", bd=2, font=("Arial", 12, "bold"), width=10, padx=8, pady=4, cursor="hand2")
    ...
    def build_attract_area(self, parent):
        ...
        self.cycle_scale = tk.Scale(left_body, from_=20, to=200, resolution=20, orient="horizontal", variable=self.cycle_seconds, bg="#17071f", fg="white", troughcolor="#071a30", highlightthickness=0, font=("Arial", 12, "bold"), command=self.on_cycle_changed, length=520)
        ...
        self.theme_brightness_scale = tk.Scale(left_body, from_=0, to=100, resolution=1, orient="horizontal", variable=self.theme_brightness_percent, bg="#17071f", fg="white", troughcolor="#071a30", highlightthickness=0, font=("Arial", 12, "bold"), command=self.on_theme_brightness_changed, length=520)
        ...
        self.game_brightness_scale = tk.Scale(left_body, from_=0, to=100, resolution=1, orient="horizontal", variable=self.gameplay_brightness_percent, bg="#17071f", fg="white", troughcolor="#071a30", highlightthickness=0, font=("Arial", 12, "bold"), command=self.on_gameplay_brightness_changed, length=520)
    ...
    def build_bottom_area(self, parent):
        ...
        self.info_text = tk.Text(info_body, height=5, width=68, font=("Arial", 18), bg="#12061f", fg="white", wrap="word", bd=0, relief="flat")
    ...
    def open_config_window(self):
        ...
        if not os.path.exists(path):
            default_payload = {"difficulty": "normal", "show_scoreboard": True, "sound_pack": "default", "notes": "auto-created by console v17.4.1; adjust as needed"}
    ...
    def open_setup_window(self):
        ...
        tk.Checkbutton(misc, text="Auto-start console on boot (suggested)", variable=self.auto_start, bg="#0f0617", fg="white", activebackground="#0f0617", activeforeground="white", selectcolor="#17071f", font=("Arial", 12, "bold")).grid(row=0, column=2, padx=12, pady=6, sticky="w")
        tk.Checkbutton(misc, text="Backup / Restore config JSON", variable=self.backup_restore, bg="#0f0617", fg="white", activebackground="#0f0617", activeforeground="white", selectcolor="#17071f", font=("Arial", 12, "bold")).grid(row=1, column=2, padx=12, pady=4, sticky="w")
        tk.Checkbutton(misc, text="Apply network settings & reboot", variable=self.apply_reboot, bg="#0f0617", fg="white", activebackground="#0f0617", activeforeground="white", selectcolor="#17071f", font=("Arial", 12, "bold")).grid(row=2, column=2, padx=12, pady=4, sticky="w")
        suggestion_box = tk.Frame(misc, bg="#0f0617")
        suggestion_box.grid(row=3, column=0, columnspan=3, sticky="ew", padx=6, pady=(6, 4))
        tk.Label(suggestion_box, text="Suggestions for setup screen:\n• Test Falcon connection (ping)\n• Apply network settings & reboot option\n• Backup/restore config JSON\n• View current IPs & link status\n• Toggle SSH on/off for support", bg="#0f0617", fg="#bbbbbb", justify="left", font=("Arial", 11)).pack(anchor="w")
    ...
    def on_start_game(self):
        ...
        if game_name in ("dot dash", ".dash", "dotdash"):
            ...
            if self.animate_enabled.get():
                ...
                self.update_animate_button()
            self.attract.stop(self)
            self.dot_dash_mapping_pending = True
            self.dot_dash_needed_players = {
                i
                for i in range(1, 5)
                if self.player_status[i]["checked_in"]
                and self.player_status[i]["state"] != "REMOVED"
                and self.controller_status[i]["enabled"]
            }
            for pid in self.dot_dash_needed_players:
                self.dot_dash_button_map[pid] = {"button_a_index": None, "button_b_index": None}
            self.apply_brightness_for_state()
            self.log("Dot Dash: all players press ANY two gameplay buttons (not white) to map A/B. No turn order; do it anytime.")
            self.refresh_player_status_panel()
            self.refresh_controller_panel()
            return
        ok, msg = game.validate_ready_to_start(self)
    ...
    def refresh_player_status_panel(self):
        ...
        for idx in range(1, 5):
            ...
            btn = tk.Button(frame, text=f"P{idx} / SLA:{self.player_status[idx]['sla']}", bg=colors[idx], fg="white", font=("Arial", 20, "bold"), relief="raised", bd=2, command=lambda i=idx: self.on_player_tile_click(i), cursor="hand2")
    ...
    def apply_saved_assignments(self):
        ...
        for slot_str, saved in self.saved_assignments.items():
            ...
            if match is None and saved_base:
                base_matches = [d for d in self.discovered_devices if d.get("base_id", "") == saved_base and d["signature"] not in assigned_signatures]
                ...
                if sig:
                    ...
                    for dev in self.discovered_devices:
                        if dev["signature"] == sig:
                            ...
                            break
                    self.assignment_map[str(slot)] = {"signature": sig, "base_id": base_id}
            if self.assignment_map:
                self.save_assignments()
        else:
            unmatched_available = [d for d in self.discovered_devices if d["signature"] not in assigned_signatures]
    ...
    def handle_assignment_press(self, js_index):
        ...
        self.assignment_map[str(slot)] = {"signature": signature, "base_id": base_id}
    ...
    def rescan_controllers(self):
        ...
        for js_index in range(count):
            try:
                ...
                self.discovered_devices.append({"js_index": js_index, "name": js.get_name(), "signature": self.controller_signature(js, js_index), "base_id": self.controller_base_id(js, js_index)})
            except Exception as e:
            ...
            for js_index, js in self.joysticks.items():
                ...
                for button_index in range(button_count):
                    ...
                    if pressed and not last_pressed:
                        ...
                        elif self.dot_dash_mapping_pending:
                            if player_num and player_num in self.dot_dash_needed_players:
                                if button_index == self.white_button_index:
                                    self.log(f"P{player_num}: white ignored during mapping.")
                                else:
                                    self._handle_dot_dash_parallel_map(player_num, button_index)
    ...
    def handle_missing_checked_in_players(self, stage_label: str) -> bool:
        ...
        continue_anyway = messagebox.askyesno("Missing Controllers", f"{stage_label}: missing controllers for {player_list}.\n\nYes = continue without those players\nNo = cancel so you can replace controller(s), scan, and REASSIGN.")
        if continue_anyway:
    ...
    def _handle_dot_dash_parallel_map(self, player_num: int, button_index: int):
        mapping = self.dot_dash_button_map[player_num]
        if mapping["button_a_index"] is None:
            mapping["button_a_index"] = button_index
            self.log(f"Dot Dash: P{player_num} mapped A to raw button {button_index}. Press a different button for B.")
            self.play_sound("button_learned")
            return
        if mapping["button_b_index"] is None:
            if button_index == mapping["button_a_index"]:
                self.log(f"Dot Dash: P{player_num} second button must be different.")
                return
            mapping["button_b_index"] = button_index
            self.log(f"Dot Dash: P{player_num} mapped B to raw button {button_index}.")
            self.play_sound("button_learned")
        if all(
            self.dot_dash_button_map[p]["button_a_index"] is not None
            and self.dot_dash_button_map[p]["button_b_index"] is not None
            for p in self.dot_dash_needed_players
        ):
            self.dot_dash_mapping_pending = False
            self.start_dot_dash_module_multi()

    def start_dot_dash_module_multi(self):
        players = []
        for pid in sorted(self.dot_dash_needed_players):
            lm = self.falcon.lane_map[pid]
            players.append(
                PlayerConfig(
                    player_id=pid,
                    name=f"Player {pid}",
        ...
        self.game_manager.start_game("dot_dash", players, settings={})
        self.active_game_key = "dot_dash"
        self.session_started = True
        self.set_state(HostState.GAME_RUNNING, "Dot Dash module started after parallel mapping.")
        self.apply_brightness_for_state()
        self.log("Dot Dash: ready. Players press their two mapped buttons to pick colors, then wait for auto countdown.")
        self.refresh_player_status_panel()
        self.refresh_controller_panel()
    ...
    def start_dot_dash_module(self, player_num: int = 1, color_a=None, color_b=None):
        ...
        self.game_manager.start_game("dot_dash", players, settings={"countdown_seconds": 3, "lane_pixel_count": 20})
