...
class PixelChallengeConsole:
    ...
    def show_scoreboard_temporarily(self, seconds: int = 6, payload=None):
        ...
        placeholder = self._scoreboard_placeholder_path()
        if os.path.exists(placeholder):
            self.viewer.show_image(placeholder)
            self.log(f"ViewerService: show scoreboard -> {placeholder}")
        else:
            self.viewer.show_black()
            self.log(f"Missing scoreboard placeholder: {placeholder}")
    ...
    def handle_dot_dash_learning_press(self, player_num: int, button_index: int) -> bool:
        ...
        if self.dot_dash_learning_stage == 2:
            ...
            self.dot_dash_learning_active = False
            self.log(
                f"Dot Dash: Player {player_num} buttons learned. "
                f"A=raw {mapping['button_a_index']}, B=raw {mapping['button_b_index']}."
            )
            self.start_dot_dash_module(player_num)
            return True
    ...
    def start_dot_dash_module(self, player_num: int = 1):
        players = [
            PlayerConfig(
        ...
        self.game_manager.handle_input(player_num, "select_color_a", (255, 0, 0))
        self.game_manager.handle_input(player_num, "select_color_b", (0, 0, 255))
    ...
    def on_start_game(self):
        ...
        if game_name in ("dot dash", ".dash", "dotdash"):
            ...
            self.log("Dot Dash: waiting for first button.")
            self.refresh_player_status_panel()
            self.refresh_controller_panel()
            return
