# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v28.6.1
    ...
    def _animate_composite_layers(self, step: int, data: dict):
        ...
        for layer in data.get("layers", []):
            ...
            for i in range(n):
                ...
                r, g, b = _hex_to_rgb(hex_c)
                dimmer_val = clamp8(int((dimmers[i] if i < len(dimmers) else 255) * self.brightness / 255))
    ...
    def _scene_slot_data_for_indexes(self, scene: dict, slot_indexes: list[int]) -> tuple[list[str], list[int], list[int]]:
        ...
        for pos, fixture_idx in enumerate(slot_indexes):
            ...
            if state:
                ...
                slot_dimmers.append(int(state.get("dimmer", 255) or 255))
                slot_strobes.append(int(state.get("strobe", 0) or 0))
            elif colors:
    ...
    def open_setup_window(self):
        ...
        def _delete_profile():
            ...
            if self.dmx_profile_id.get() == pid:
                next_profile = self.dmx_profiles.get("profiles", [])[0] if self.dmx_profiles.get("profiles") else None
                if next_profile:
                    self.dmx_profile_id.set(next_profile.get("id", ""))
                    selected_display.set(f"{next_profile.get('manufacturer','')} - {next_profile.get('model','')}")
                    self._sync_profile_runtime_to_vars(next_profile)
...
• Toggle SSH on/off for support"""
    ...
    def _open_add_profile_dialog(self, profile_listbox, profile_combo,
        ...
        channels_var = tk.StringVar(value="8")
        ...
        def _save_profile():
            ...
            for d in new_display:
                profile_listbox.insert("end", d)
            profile_combo.configure(values=new_display)
            selected_display.set(f"{new_profile.get('manufacturer','')} - {new_profile.get('model','')}")
            self.dmx_profile_id.set(pid)
            self._sync_profile_runtime_to_vars(new_profile)
