...
class PixelChallengeConsole:
    ...
    def apply_saved_assignments(self):
        ...
        for slot_str, saved in self.saved_assignments.items():
            ...
            if match is None and saved_base:
                base_matches = [d for d in self.discovered_devices if d["base_id"] == saved_base and d["signature"] not in assigned_signatures]
            ...
            for slot in range(1, 5):
                ...
                if sig:
                    self.assignment_map[str(slot)] = {
                        "signature": sig,
                        "base_id": self.discovered_devices[slot - 1]["base_id"] if slot - 1 < len(self.discovered_devices) else "",
                    }
    ...
    def handle_missing_checked_in_players(self, stage_label: str) -> bool:
        ...
        continue_anyway = messagebox.askyesno(
            ...
            f"No = cancel so you can replace controller(s), rescan, and REASSIGN."
        )
