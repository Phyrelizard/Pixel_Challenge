...
class PixelChallengeConsole:
    def __init__(self, root: tk.Tk):
        ...
        # Set this to the actual button index used for the WHITE button
    ...
    def init_joysticks(self):
        ...
        for js_index in range(count):
            try:
                ...
                self.log(f"Joystick js{js_index} detected: {js.get_name()}")
            except Exception as e:
    ...
    def handle_white_button_press(self, player_num: int, js_index: int):
        ...
        if self.checkin_open and not self.session_started:
            ...
            self.log(f"Ignored join input from Player {player_num}: session already active.")
        else:
            self.log(f"Ignored join input from Player {player_num}: check-in is not open.")
