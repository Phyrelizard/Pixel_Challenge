...
class FalconService:
    ...
    def _build_frame(self, pixels):
        ...
        for i in range(max_pixels):
            r, g, b = pixels[i]
            base = i * 3
            buf[base + 0] = max(0, min(255, int(r)))
            buf[base + 1] = max(0, min(255, int(g)))
            buf[base + 2] = max(0, min(255, int(b)))
        return bytes(buf)

    def _send_pixels(self, universe: int, pixels):
        frame = self._build_frame(pixels)
        self.sender[universe].dmx_data = frame

    def _blank_pixels(self):
    ...
    def clear_all_lanes(self, host):
        for player_id in self.lane_map:
            for lane in ("left", "right"):
                ...
                self._send_pixels(universe, self._blank_pixels())
        host.log("FalconService: all lanes cleared.")

    def set_lane_off(self, host, player_id: int, lane: str):
        universe = self.lane_map[player_id][lane]
        self._send_pixels(universe, self._blank_pixels())
        host.log(f"FalconService: P{player_id} {lane} OFF -> U{universe}")

    def set_lane_solid(self, host, player_id: int, lane: str, color_name: str):
        universe = self.lane_map[player_id][lane]
        color = COLOR_MAP.get(color_name.lower(), (255, 255, 255))
        pixels = [color] * self.pixels_per_lane
        self._send_pixels(universe, pixels)
        host.log(f"FalconService: P{player_id} {lane} SOLID {color_name} -> U{universe}")

    def set_lane_pixel(self, host, player_id: int, lane: str, index: int, color_name: str):
        universe = self.lane_map[player_id][lane]
        color = COLOR_MAP.get(color_name.lower(), (255, 255, 255))
        pixels = self._blank_pixels()
        if 0 <= index < self.pixels_per_lane:
            pixels[index] = color
        self._send_pixels(universe, pixels)
        host.log(f"FalconService: P{player_id} {lane} PIXEL {index} {color_name} -> U{universe}")

    def set_lane_dash(self, host, player_id: int, lane: str, index: int, color_name: str, length: int = 3):
        universe = self.lane_map[player_id][lane]
        color = COLOR_MAP.get(color_name.lower(), (255, 255, 255))
        pixels = self._blank_pixels()
        start = max(0, index)
        end = min(self.pixels_per_lane, index + length)
        for i in range(start, end):
            pixels[i] = color
        self._send_pixels(universe, pixels)
        host.log(f"FalconService: P{player_id} {lane} DASH {index}:{end-1} {color_name} -> U{universe}")

    def run_attract_theme(self, host, theme_name: str):
        theme_name = theme_name.lower()
        if theme_name == "rainbow pulse":
            colors = ["red", "orange", "yellow", "green", "blue", "purple", "cyan", "white"]
        elif theme_name == "fire burst":
            colors = ["red", "orange", "yellow", "red", "orange", "yellow", "red", "orange"]
        elif theme_name == "ice burst":
            colors = ["blue", "cyan", "white", "blue", "cyan", "white", "blue", "cyan"]
        elif theme_name == "galaxy wave":
            colors = ["purple", "blue", "cyan", "purple", "blue", "cyan", "purple", "white"]
        elif theme_name == "team colors":
            colors = ["red", "green", "blue", "orange", "white", "red", "green", "blue"]
        else:  # Calm Mode
            colors = ["blue", "blue", "cyan", "cyan", "purple", "purple", "white", "white"]

        idx = 0
        for player_id in range(1, 5):
            for lane in ("left", "right"):
                self.set_lane_solid(host, player_id, lane, colors[idx])
                idx += 1
        host.log(f"FalconService: attract theme '{theme_name}' pushed to Falcon.")

    def all_lanes_test(self, host):
        test_colors = {
        ...
        for player_id in range(1, 5):
            self.set_lane_solid(host, player_id, "left", test_colors[player_id]["left"])
            self.set_lane_solid(host, player_id, "right", test_colors[player_id]["right"])
        host.log("FalconService: all lanes test sent.")
...
class AttractService:
    ...
    def start_theme(self, host, theme_name: str):
        ...
        self.falcon.run_attract_theme(host, theme_name)
...
class BaseGameModule:
    ...
    def validate_ready_to_start(self, host) -> tuple[bool, str]:
    ...
    def update(self, host, dt_ms: int):
        pass
...
class DotDashModule(BaseGameModule):
    ...
    def get_intro_video_path(self) -> str:
        return "/home/ledgame/easter_game/assets/dot_dash_intro.mp4"

    def validate_ready_to_start(self, host) -> tuple[bool, str]:
        ok, msg = super().validate_ready_to_start(host)
        if not ok:
            return ok, msg
        return True, ""
...
class GameRegistry:
    ...
    def get(self, game_name: str) -> BaseGameModule:
...
class PixelChallengeConsole:
    ...
    def current_game(self) -> BaseGameModule:
    ...
    def init_joysticks(self):
        ...
        self.log(f"Detected {count} joystick(s).")
        ...
        for js_index in range(count):
            try:
                ...
                player_num = js_index + 1
                if player_num <= 4:
                    self.joystick_player_map[js_index] = player_num
                    self.log(f"P{player_num}: js{js_index} | {js.get_name()}")
            except Exception as e:
    ...
    def handle_white_button_press(self, player_num: int, js_index: int):
        self.log(f"White button pressed on js{js_index} (Player {player_num}).")
    ...
    def toggle_animate(self):
        ...
        if self.animate_enabled.get():
            self.attract.start_theme(self, self.theme_names[self.theme_listbox.curselection()[0]])
        else:
    ...
    def on_view_intro(self):
        ...
        if not video_path:
            messagebox.showinfo("View Intro", f"No intro clip assigned yet for {self.selected_game.get()}.")
            self.log(f"No intro assigned for {self.selected_game.get()}.")
            return
    ...
    def on_start_game(self):
        ...
        self.refresh_player_status_panel()
        self.refresh_controller_panel()
        messagebox.showinfo("Start Game", f"Start {game.get_name()}.")
    ...
    def on_all_lanes_test(self):
        self.falcon.all_lanes_test(self)
        messagebox.showinfo("All Lanes Test", "All 8 Falcon universes have been driven with test colors.")
    ...
    def on_player_checkin(self):
        ...
        if self.host_state == HostState.CHECKIN_OPEN:
            ...
            messagebox.showinfo("Player Check-In", "Check-in is open. Players can now press their WHITE button to join.")
