...
class FalconService:
    ...
    def clear_all_lanes(self, host):
        host.log("FalconService: clear all lanes (stub).")

    def set_lane_off(self, host, player_id: int, lane: str):
        universe = self.lane_map[player_id][lane]
        host.log(f"FalconService: P{player_id} {lane} -> OFF (U{universe}) [stub].")

    def set_lane_solid(self, host, player_id: int, lane: str, color_name: str):
        universe = self.lane_map[player_id][lane]
        host.log(f"FalconService: P{player_id} {lane} -> {color_name} (U{universe}) [stub].")

    def run_attract_theme(self, host, theme_name: str):
        host.log(f"FalconService: run attract theme '{theme_name}' [stub].")
...
class PixelChallengeConsole:
    ...
    def can_join_players(self) -> bool:
        return self.host_state in {HostState.IDLE, HostState.CHECKIN_OPEN}
    ...
    def on_all_lanes_test(self):
        ...
        messagebox.showinfo("All Lanes Test", "Falcon lane diagnostics requested (stub).")
