...
class FalconService:
    ...
    def render_theme_frame(self, theme_name: str, step: int):
        ...
        for player_id in range(1, 5):
            for lane_idx, lane in enumerate(("left", "right")):
                slot = (player_id - 1) * 2 + lane_idx
                pixels = self._theme_pixels(theme_name, slot, step)
...
class PixelChallengeConsole:
    def __init__(self, root: tk.Tk):
        ...
        self.controller_status = {
            1: {"enabled": False, "locked": False, "selected": False, "status": "MISSING", "name": ""},
            2: {"enabled": False, "locked": False, "selected": False, "status": "MISSING", "name": ""},
            3: {"enabled": False, "locked": False, "selected": False, "status": "MISSING", "name": ""},
            4: {"enabled": False, "locked": False, "selected": False, "status": "MISSING", "name": ""},
        }
        ...
        self.info_lines = [
            "P1 | js0 | Left: U1  Right: U2",
            "P2 | js1 | Left: U3  Right: U4",
            "P3 | js2 | Left: U5  Right: U6",
            "P4 | js3 | Left: U7  Right: U8",
            "Host boot complete.",
        ]
        ...
        self.root.after(ANIMATION_INTERVAL_MS, self.animation_tick)
    ...
    def controller_connected(self, player_id: int) -> bool:
        return player_id in self.joystick_player_map.values()
        ...
        for js_index in range(count):
            try:
                ...
                if player_num <= 4:
                    ...
                    self.controller_status[player_num]["enabled"] = True
                    self.controller_status[player_num]["status"] = "ONLINE"
                    self.controller_status[player_num]["name"] = js.get_name()
    ...
    def refresh_controller_panel(self):
        ...
        for idx in range(1, 5):
            ...
            if data["name"]:
                tk.Label(inner, text=data["name"], bg="#0f0617", fg="#cccccc", font=("Arial", 10), wraplength=180, justify="center").pack(pady=(0, 8))
    ...
    def toggle_animate(self):
        ...
        self.animate_btn.configure(
            ...
            bg="#58be3d" if enabled else "#888888",
            activebackground="#58be3d" if enabled else "#888888",
        )
