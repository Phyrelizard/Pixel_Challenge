...
class PixelChallengeConsole:
    ...
    def apply_saved_assignments(self):
        ...
        for slot_str, signature in self.saved_assignments.items():
            try:
                slot = int(slot_str)
            except Exception:
                continue
            ...
            for item in self.discovered_devices:
                if item["signature"] == signature and item["signature"] not in assigned_signatures:
                    match = item
                    break
        ...
        unassigned_slots = [slot for slot in range(1, 5) if not self.controller_status[slot]["signature"]]
        unassigned_devices = [d for d in self.discovered_devices if d["signature"] not in assigned_signatures]

        for slot, device in zip(unassigned_slots, unassigned_devices):
            self.controller_status[slot]["name"] = device["name"]
            self.controller_status[slot]["signature"] = device["signature"]
            if not self.controller_status[slot]["locked"]:
                self.controller_status[slot]["status"] = "ONLINE"
                self.controller_status[slot]["enabled"] = True
    ...
    def start_assignment_mode(self):
        ...
        self.assignment_used_signatures = set()
        self.assignment_map = {}
        self.log("Controller assignment mode started.")
    ...
    def finish_assignment_mode(self):
        ...
        self.log("Controller assignment mode complete.")
    ...
    def handle_assignment_press(self, js_index):
        ...
        self.assignment_map[str(slot)] = signature
        ...
        if self.assignment_step > 4:
            ...
            self.log(f"Press WHITE on the controller you want to assign to Controller {self.assignment_step}.")
    ...
    def build_top_bar(self):
        ...
        center.grid(row=0, column=1, sticky="", padx=40)
    ...
    def refresh_controller_panel(self):
        ...
        self.neon_button(footer, "TEST", self.on_test_controller, bg="#1b63ff", width=8).pack(side="left", padx=5)
        self.neon_button(footer, "SCAN", self.on_scan_controllers, bg="#1b63ff", width=8).pack(side="left", padx=5)
        self.neon_button(footer, "ASSIGN", self.on_assign_controllers, bg="#1b63ff", width=8).pack(side="left", padx=5)
        ...
        self.neon_button(footer, toggle_text, self.on_enable_all, bg=toggle_bg, width=12).pack(side="left", padx=5)
    ...
    def on_assign_controllers(self):
        self.rescan_controllers()
        self.start_assignment_mode()

    def on_test_controller(self):
        self.log("Controller rescan requested.")
        self.rescan_controllers()
