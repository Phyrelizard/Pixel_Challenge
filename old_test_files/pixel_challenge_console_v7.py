...
class PixelChallengeConsole:
        ...
        for js_index in range(count):
            try:
                ...
                if player_num <= 4:
                    ...
                    self.log(f"Extra joystick js{js_index} ignored: {js.get_name()}")
            except Exception as e:
    ...
    def handle_white_button_press(self, player_num: int, js_index: int):
        ...
        if self.checkin_open and not self.session_started:
            self.simulate_player_join(player_num)
        elif self.session_started:
            self.log(f"Ignored white-button join from Player {player_num}: session already active.")
        else:
    ...
    def on_view_intro(self):
        ...
        intro_map = {
            "Dot Dash": "/home/ledgame/easter_game/assets/dot_dash_test_intro.mp4",
            "Pixel Pop": "",
            "Surround": "",
            "Ascend": "",
        }

        video_path = intro_map.get(game, "")

        if not video_path:
            messagebox.showinfo("View Intro", f"No intro clip assigned yet for {game}.")
            self.log(f"No intro assigned for {game}.")
            return

        try:
            with open(self.command_file, "w", encoding="utf-8") as f:
                f.write(f"PLAY_VIDEO|{video_path}\n")

            self.log(f"Requested intro for {game}: {video_path}")
        except Exception as e:
            messagebox.showerror("View Intro", f"Failed to send intro command:\n{e}")
            self.log(f"Intro command failed for {game}: {e}")
    ...
    def on_player_checkin(self):
        ...
        if self.checkin_open:
            ...
            messagebox.showinfo(
                "Player Check-In",
                "Check-in is open. Players can now press their WHITE button to join.",
            )
        else:
