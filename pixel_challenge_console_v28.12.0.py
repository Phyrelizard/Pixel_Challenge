# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v28.12.0
...
class FalconService:
    ...
    def _flame_theme_pixels(self, theme_name: str, lane_slot: int, step: int):
        ...
        t = step * 0.145 * cfg["speed"]

        height_noise = self._smooth_wave(t * 1.20, seed)
        height = cfg["base"] + cfg["swing"] * (height_noise - 0.5) * 2.0
        height = max(0.16, min(0.92, height))

        # Occasional deterministic flare/dip.  This is small enough to feel
        # like a flame flutter, not a whole-lane blink.
        flare_wave = math.sin((step * 0.39 * cfg["speed"]) + seed * 3.17)
        if flare_wave > 0.88:
            height += (flare_wave - 0.88) * 0.30
        elif flare_wave < -0.93:
            height -= (-0.93 - flare_wave) * 0.22
        height = max(0.14, min(0.96, height))
        ...
        for i in range(n):
            ...
            tip_wiggle = (
                0.055 * math.sin((step * 0.30 * cfg["speed"]) + seed + y * 9.0)
                + 0.035 * math.sin((step * 0.73 * cfg["speed"]) + seed * 0.4 + y * 21.0)
            )
            local_height = max(0.08, min(1.0, height + tip_wiggle))
            ...
            shimmer = 0.86 + 0.14 * math.sin((step * 0.86 * cfg["speed"]) + seed * 1.31 + y * 17.0)
            tip_flutter = 1.0 + (0.22 * math.sin((step * 1.10 * cfg["speed"]) + seed * 2.1 + y * 31.0) * max(0.0, pos - 0.58))
            level = (cfg["floor"] + (body ** 0.55) * 0.94) * shimmer * tip_flutter

            # Small bright lick that travels through the flame body, different
            # for every lane slot.
            lick_center = 0.18 + 0.55 * self._smooth_wave((step * 0.10 * cfg["speed"]) + seed * 0.2, seed * 1.7)
            lick = max(0.0, 1.0 - abs(pos - lick_center) / 0.12)
            level += lick * 0.16

            # Rare little spark/hot pop near the upper body.
            spark_phase = math.sin((step * 0.47 * cfg["speed"]) + seed * 4.7)
            if spark_phase > (1.0 - cfg["spark"]):
                spark_pos = 0.55 + 0.32 * self._smooth_wave((step * 0.21) + seed, seed * 3.2)
                spark = max(0.0, 1.0 - abs(pos - spark_pos) / 0.035)
                level += spark * 0.45
