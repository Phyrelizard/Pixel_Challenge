# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v16.8.0
- Panels are resizable via PanedWindow; sash positions persist across reboots.
  * Outer horizontal split: Attract pane (left) vs Main area (right)
  * Inner horizontal split in Main: Player/Status column vs Controllers column
  * Bottom split: Main upper area vs Info window
- Per-theme speed sliders (persist), no global speed factor.
- CYCLE/ANIMATE gating unchanged.
"""
import os
import json
import time
...
# min sizes for panes to avoid collapse
MIN_LEFT = 340
MIN_CENTER = 600
MIN_CONTROLLERS = 360
MIN_INFO_HEIGHT = 150
...
class ViewerService:
    ...
    def show_final_results(self):
        self._write("SHOW_FINAL_RESULTS")  # placeholder hook
...
class PixelChallengeConsole:
    ...
    def cycle_allowed(self):
        return (
            self.cycle_enabled.get()
            and self.lights_should_run()
            and not self.host_state == HostState.GAME_RUNNING
        )
    ...
    def build_ui(self):
        ...
        # Main vertical split: upper/lower (info)
        ...
        # Upper content frame (will hold horizontal splits)
        ...
        # Horizontal split: left (attract) vs right (main columns)
        ...
        # Horizontal split inside right_container: center vs controllers
        ...
        # restore sash positions if present
        self.root.update_idletasks()
        if self.sash_left_main is not None:
            try:
                self.main_horizontal.sash_place(0, int(self.sash_left_main), 0)
            except Exception:
                pass
        if self.sash_center_ctrl is not None:
            try:
                self.right_horizontal.sash_place(0, int(self.sash_center_ctrl), 0)
            except Exception:
                pass
        if self.sash_main_info is not None:
            try:
                self.main_vertical.sash_place(0, 0, int(self.sash_main_info))
            except Exception:
                pass

        # Bind sash move events to persist
    ...
    def build_attract_area(self, parent):
        ...
        self.lanes_test_btn.pack(fill="x", pady=(4, 2))
        self.update_lanes_test_button()
    ...
    def handle_dot_dash_learning_press(self, player_num: int, button_index: int) -> bool:
        ...
        if self.dot_dash_learning_stage == 1:
            ...
            self.log(f"Dot Dash: Player {player_num} first button learned as raw button {button_index}. Now press the SECOND game button.")
            return True
