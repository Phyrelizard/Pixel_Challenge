# -*- coding: utf-8 -*-
"""
Pixel Challenge Host Console v28.26.17
...
CONTROLLER_HELP_CARD_PROFILES = {
    "voyee_s08": {
        "display_name": "VOYEE",
        "join_hint": "Check-in opened. Arcade WHITE or VOYEE L/Menu to join.",
        "checkin_image": "controller_help_voyee_s08_checkin.png",
        "select_colors_image": "controller_help_voyee_s08_select_colors.png",
        "name_tokens": ("voyee", "s08"),
    },
    "switch_style": {
        "display_name": "Gamepad",
        "join_hint": "Check-in opened. Arcade WHITE or Gamepad L/Menu to join.",
        "checkin_image": "controller_help_voyee_s08_checkin.png",
        "select_colors_image": "controller_help_voyee_s08_select_colors.png",
        "name_tokens": ("switch", "pro controller"),
    },
}
...
class ViewerService:
    ...
    def show_select_colors(self, image_path: str = None):
        ...
        image_path = image_path or f"{ASSETS_DIR}/select_two_colors.png"
        self._write(f"SHOW_IMAGE|{image_path}")

    def show_checkin(self, image_path: str = None):
        """Show player check-in screen, optionally with a controller-specific help card."""
        image_path = image_path or f"{ASSETS_DIR}/player_check-in.png"
        self._write(f"SHOW_IMAGE|{image_path}")
...
class FalconService:
    def __init__(self, falcon_ip: str, pixels_per_lane: int = 100, dmx_universe: int = None, playfield_inverted: bool = False,
        ...
        # Ultra-dim output dithering preserves low RGB channels after the global
        # Falcon brightness scaler would otherwise truncate them to 0.  Keep the
        # dither phase uniform across a whole universe/channel so it cannot form
        # moving dots or upward streaks on dim background glow.
        self._output_dither_cycle = 32
    ...
    def _dither_scaled_channel(self, value: float, scale: float, universe: int, pixel_index: int, channel_index: int) -> int:
        """Scale one RGB channel while preserving ultra-dim color channels.

        v28.19.1 used a pixel-indexed temporal dither.  That preserved dim
        purple better, but it also created visible moving dots/streaks because
        the dither phase advanced along the lane.  This version only dithers
        the ultra-dim range and uses one uniform phase per universe/channel, so
        the background glow can average dimmer without crawling upward.
        """
        scaled = max(0.0, min(255.0, float(value) * float(scale)))
        if scaled <= 0.0:
            return 0

        # Normal/brighter pixels do not need sub-step dithering.  Rounding keeps
        # ordinary gameplay colors stable and avoids unnecessary shimmer.
        if scaled >= 4.0:
            return clamp8(int(round(scaled)))
        ...
        cycle = max(2, int(getattr(self, "_output_dither_cycle", 32)))
        threshold = max(0, min(cycle, int(round(frac * cycle))))
        if threshold <= 0 or whole >= 255:
            return clamp8(whole)

        # Important: do not include pixel_index here.  A pixel-indexed phase
        # creates diagonal/upward motion on vertical lanes.  One phase per
        # universe/channel produces a steady field instead of traveling dots.
        frame = int(time.monotonic() * 120.0)
        phase = (frame + int(universe) * 5 + int(channel_index) * 11) % cycle
        return clamp8(whole + (1 if phase < threshold else 0))

    def _build_frame(self, pixels, universe: int = 0):
        buf = bytearray(512)
        ...
        for i in range(max_pixels):
            r, g, b = pixels[i]
            base = i * 3
            buf[base + 0] = self._dither_scaled_channel(r, scale, universe, i, 0)
            buf[base + 1] = self._dither_scaled_channel(g, scale, universe, i, 1)
            buf[base + 2] = self._dither_scaled_channel(b, scale, universe, i, 2)
        return bytes(buf)
    ...
    def _send_pixels(self, universe: int, pixels):
        self.send_universe_frame(universe, self._build_frame(pixels, universe))
...
class GameRegistry:
    def __init__(self):
        self.games = {
            ...
            "Chomp Chase": PlaceholderGameModule("Chomp Chase"),
        }
...
class PixelChallengeConsole:
    def __init__(self, root: tk.Tk):
        ...
        self.root.geometry("1600x900+100+80")
        ...
        # Looping SFX channels for continuous gameplay sounds such as Ascend movement.
        self._looping_sound_channels = {}
        self._looping_sound_objects = {}
        ...
        self._music_paused_by_game_pause = False
        ...
        # v28.26.12: clean boot/reboot starts on the Pixel Challenge Home/Splash
        # screen instead of advancing to the first playable game tile.
...
"""
    ...
    def play_sound(self, sound_key: str):
        ...
        sound_paths = {
            ...
            "as_music_victory": project_path("assets/audio/ascend/as_music_victory.ogg"),
            "as_band_build": project_path("assets/audio/ascend/as_band_build.wav"),
            "as_band_build_start": project_path("assets/audio/ascend/as_band_build_start.wav"),
            "as_band_build_tick": project_path("assets/audio/ascend/as_band_build_tick.wav"),
            "as_band_build_tick_soft": project_path("assets/audio/ascend/as_band_build_tick_soft.wav"),
            "as_band_build_complete": project_path("assets/audio/ascend/as_band_build_complete.wav"),
            "as_move_forward": project_path("assets/audio/ascend/as_move_forward.wav"),
            "as_move_backward": project_path("assets/audio/ascend/as_move_backward.wav"),
            "as_jump": project_path("assets/audio/ascend/as_jump.wav"),
            "as_land": project_path("assets/audio/ascend/as_land.wav"),
            "as_player_hit": project_path("assets/audio/ascend/as_player_hit.wav"),
            "as_warp": project_path("assets/audio/ascend/as_warp.wav"),
            "as_wall_build": project_path("assets/audio/ascend/as_wall_build.wav"),
            "as_wall_build_start": project_path("assets/audio/ascend/as_wall_build_start.wav"),
            "as_wall_build_tick": project_path("assets/audio/ascend/as_wall_build_tick.wav"),
            "as_wall_build_complete": project_path("assets/audio/ascend/as_wall_build_complete.wav"),
            "as_laser_fire": project_path("assets/audio/ascend/as_laser_fire.wav"),
            "as_wall_hit": project_path("assets/audio/ascend/as_wall_hit.wav"),
            "as_wall_break": project_path("assets/audio/ascend/as_wall_break.wav"),
            "as_wall_miss": project_path("assets/audio/ascend/as_wall_miss.wav"),
            "as_launch": project_path("assets/audio/ascend/as_launch.wav"),
            "as_glass_break": project_path("assets/audio/ascend/as_glass_break.wav"),
            "as_game_over": project_path("assets/audio/ascend/as_game_over.wav"),
            "as_winner": project_path("assets/audio/ascend/as_winner.wav"),

            # Ascend logical event aliases (safe fallback if a stale game module sends logical keys).
            "music_gameplay": project_path("assets/audio/ascend/as_music_gameplay.ogg"),
            "leg_start": project_path("assets/audio/ascend/as_band_build_start.wav"),
            "intro_build": project_path("assets/audio/ascend/as_band_build_start.wav"),
            "move_forward": project_path("assets/audio/ascend/as_move_forward.wav"),
            "move_backward": project_path("assets/audio/ascend/as_move_backward.wav"),
            "jump": project_path("assets/audio/ascend/as_jump.wav"),
            "land": project_path("assets/audio/ascend/as_land.wav"),
            "hit": project_path("assets/audio/ascend/as_player_hit.wav"),
            "warp": project_path("assets/audio/ascend/as_warp.wav"),
            "wall_build": project_path("assets/audio/ascend/as_wall_build_start.wav"),
            "fire": project_path("assets/audio/ascend/as_laser_fire.wav"),
            "wall_hit": project_path("assets/audio/ascend/as_wall_hit.wav"),
            "wall_miss": project_path("assets/audio/ascend/as_wall_miss.wav"),
            "launch": project_path("assets/audio/ascend/as_launch.wav"),
            "glass_break": project_path("assets/audio/ascend/as_glass_break.wav"),
    ...
    def _resolve_looping_sound_path(self, sound_key: str) -> str:
        """Resolve supported looping SFX keys to file paths.

        Keep this intentionally small: gameplay music still uses pygame.music,
        while short continuous effects such as Ascend movement get their own
        looping mixer channel so they do not restart with audible gaps.
        """
        paths = {
            "as_move_forward": project_path("assets/audio/ascend/as_move_forward.wav"),
            "move_forward": project_path("assets/audio/ascend/as_move_forward.wav"),
        ...
        return paths.get(str(sound_key), "")

    def play_looping_sound(self, sound_key: str):
        """Start a looping sound effect by key if it is not already playing."""
        sound_key = str(sound_key)
        try:
            existing = getattr(self, "_looping_sound_channels", {}).get(sound_key)
            if existing is not None and existing.get_busy():
                return
        except Exception:
            pass

        path = self._resolve_looping_sound_path(sound_key)
        if not path:
            if self.debug_logging.get():
                self.log(f"[AUDIO] Unknown looping sound key: {sound_key}")
            return
        if not os.path.exists(path):
            if self.debug_logging.get():
                self.log(f"[AUDIO] Looping file not found: {path}")
            return
        ...
        try:
            if not pygame.mixer.get_init():
                ...
                pygame.mixer.set_num_channels(16)
            master = max(0, min(100, self.master_volume.get())) / 100.0
            sound = self._looping_sound_objects.get(sound_key)
            if sound is None:
                sound = pygame.mixer.Sound(path)
                self._looping_sound_objects[sound_key] = sound
            sound.set_volume((self.sfx_volume.get() / 100.0) * master)
            channel = pygame.mixer.find_channel(True)
            if channel is not None:
                channel.play(sound, loops=-1)
                self._looping_sound_channels[sound_key] = channel
                if self.debug_logging.get():
                    self.log(f"[AUDIO] Looping sound started: {sound_key}")
        except Exception as e:
            if self.debug_logging.get():
                self.log(f"[AUDIO] Looping play error for {sound_key}: {e}")

    def stop_looping_sound(self, sound_key: str):
        """Stop a looping sound effect by key."""
        sound_key = str(sound_key)
        try:
            channel = getattr(self, "_looping_sound_channels", {}).pop(sound_key, None)
            if channel is not None:
                channel.stop()
                if self.debug_logging.get():
                    self.log(f"[AUDIO] Looping sound stopped: {sound_key}")
        except Exception as e:
            if self.debug_logging.get():
                self.log(f"[AUDIO] Looping stop error for {sound_key}: {e}")

    def stop_all_looping_sounds(self):
        """Stop all active looping SFX channels."""
        try:
            for key in list(getattr(self, "_looping_sound_channels", {}).keys()):
                self.stop_looping_sound(key)
        except Exception:
            pass
    ...
    def apply_brightness_for_state(self):
        ...
        if self.host_state in (HostState.GAME_SETUP, HostState.COUNTDOWN, HostState.GAME_RUNNING, HostState.GAME_PAUSED):
    ...
    def set_state(self, new_state: HostState, reason: str = ""):
        ...
        self.refresh_start_pause_button()
        self.apply_brightness_for_state()
    ...
    def show_external_carousel(self, active: str = "current_game", ensure_playable: bool = False, play_preview_audio: bool = True):
        ...
        payload = {
            ...
            "items": items,
            # v28.26.17: When the carousel is summoned on top of gameplay, hiding
            # the tile band should restore the gameplay artwork.  In idle/front-end
            # mode, hiding the tiles should preserve the centered tile splash instead
            # of falling back to the previous game's pre-carousel image.
            "preserve_existing_background": bool(gameplay_like_state),
        }
        try:
    ...
    def force_wii_laptop_control(self, reason: str = ""):
        """Ask the Wii menu wand helper to stay in LAPTOP/console mouse mode.

        v28.26.13: Opening Check-In from the console is an operator/laptop task.
        Do not let the regular external-GSV preference pull focus back to the
        public monitor while the operator is trying to click Check-In or join
        players.
        """
        try:
            payload = {
            ...
            with open(tmp, "w", encoding="utf-8") as f:
                json.dump(payload, f, indent=2)
            os.replace(tmp, path)
            self.log(f"Wii Menu Wand <- force LAPTOP control ({reason or 'console_request'})")
        except Exception as e:
            self.log(f"Wii Menu Wand force-laptop command failed: {e}")

    def _current_external_carousel_active_id(self):
        """Return the centered GSV tile id when the viewer reports it.

        v28.26.16: Wii Home acts like a toggle.  To return to the exact tile
        the operator was on before Home, prefer the viewer's active tile status.
        Fall back to the selected game tile if the status file is not current.
        """
        try:
            st = self._read_json_file_safe(project_path("gsv_status.json"))
            updated = float(st.get("updated_at", 0) or 0)
            if time.time() - updated < 5.0 and st.get("carousel_visible"):
                active = str(st.get("carousel_active_id", "") or "").strip()
                if active:
                    return active
        except Exception:
            pass
        try:
            return self.resolve_external_carousel_active_id("current_game")
        except Exception:
            return "home"

    def _handle_wii_home_toggle(self):
        """Home button toggles between Pixel Challenge Home and the prior tile."""
        try:
            if getattr(self, "wii_home_showing", False) and getattr(self, "wii_home_return_active_id", None):
                active_id = str(getattr(self, "wii_home_return_active_id", "") or "").strip() or "current_game"
                selected = str(getattr(self, "wii_home_return_selected_game", "") or "").strip()
                playable = self.external_playable_game_names()
                if selected in playable:
                    self.selected_game.set(selected)
                self.current_intro_index = -1
                self.final_results_active = False
                self.cancel_viewer_return()
                self.show_external_carousel(active=active_id, ensure_playable=False)
                self.wii_home_showing = False
                self.log(f"Wii Home toggle: returned to previous tile {active_id}")
                return

            current_active = self._current_external_carousel_active_id()
            current_selected = self.selected_game.get()
            if current_active and current_active != "home":
                self.wii_home_return_active_id = current_active
                self.wii_home_return_selected_game = current_selected

            self.cancel_viewer_return()
            self.selected_game.set("Splash")
            self.current_intro_index = -1
            self.show_external_carousel(active="home", ensure_playable=False)
            self.wii_home_showing = True
            self.log(f"Wii Home toggle: Pixel Challenge Home; saved return tile {getattr(self, 'wii_home_return_active_id', '')}")
        except Exception as e:
            self.log(f"Wii Home toggle error: {e}")
    ...
    def handle_external_menu_action(self, action: str):
        ...
        if action in ("laptop_active", "set_laptop", "laptop", "phone_laptop_active", "phone_laptop", "phone_set_laptop"):
            self.external_gsv_preferred = False
            source = "PHONE TOUCHPAD" if action.startswith("phone_") else "WII/PHONE"
            self.set_input_screen_mode("laptop", source)
            try:
            ...
            self.log(f"External carousel: Laptop console active via {source}; external tiles hidden; splash/music preserved")
            return
        ...
        if action.startswith("preview_game|") or action.startswith("preview_non_game|"):
            ...
            # If the operator manually scrolls away from Home, the next Home press
            # should save the newly centered tile instead of treating it as the
            # second half of the Home toggle.
            if action.startswith("preview_game|") or action_raw.lower() not in ("preview_non_game|home",):
                self.wii_home_showing = False
        ...
        if action in ("home", "home_toggle"):
            self._handle_wii_home_toggle()
            return
    ...
    def handle_wii_volume_command(self, action: str):
        """Handle Wii Remote +/- master volume commands from wii_menu_wand.py.
        ...
        Plus/minus should control the existing MASTER volume fader.  If MASTER
        is muted, any + or - activity restores the previous master level first.
        """
        action = (action or "").strip().lower()
        step = 5
        try:
            # Allow a future payload such as "up|10" without breaking today.
            parts = action.split("|")
            action = parts[0]
            if len(parts) > 1:
                step = max(1, min(25, int(float(parts[1]))))
        except Exception:
            step = 5

        def restore_if_muted():
            if self.master_muted or int(self.master_volume.get()) <= 0:
                restore = int(getattr(self, "master_volume_before_mute", 100) or 100)
                restore = max(1, min(100, restore))
                self.master_muted = False
                self.master_volume.set(restore)
                self.on_master_volume_changed(restore)
                self.update_master_mute_button()
                return True
            return False

        if action in ("up", "+", "increase"):
            # v28.26.16: if muted, +/- acts as UNMUTE ONLY and restores the
            # exact pre-mute master level.  Do not also apply a + step, or a
            # 90% mute would come back as 95%.
            if restore_if_muted():
                self.log(f"Wii Remote: master volume unmuted -> {int(self.master_volume.get())}%")
                return
            new_vol = max(0, min(100, int(self.master_volume.get()) + step))
            self.master_volume.set(new_vol)
            self.on_master_volume_changed(new_vol)
            self.log(f"Wii Remote: master volume up -> {new_vol}%")
            return

        if action in ("down", "-", "decrease"):
            # v28.26.16: if muted, +/- acts as UNMUTE ONLY and restores the
            # exact pre-mute master level.  Do not also apply a - step, or a
            # 90% mute would come back as 85%.
            if restore_if_muted():
                self.log(f"Wii Remote: master volume unmuted -> {int(self.master_volume.get())}%")
                return
            new_vol = max(0, min(100, int(self.master_volume.get()) - step))
            self.master_volume.set(new_vol)
            self.on_master_volume_changed(new_vol)
            self.log(f"Wii Remote: master volume down -> {new_vol}%")
            return

        if action in ("mute", "toggle_mute"):
            if not self.master_muted:
                if int(self.master_volume.get()) > 0:
                    self.master_volume_before_mute = int(self.master_volume.get())
                self.master_muted = True
                self.master_volume.set(0)
                self.on_master_volume_changed(0)
                self.update_master_mute_button()
                self.log("Wii Remote: master volume muted")
            return

        if action in ("unmute", "restore"):
            restore_if_muted()
            self.log(f"Wii Remote: master volume unmuted -> {int(self.master_volume.get())}%")
            return

        self.log(f"Unknown Wii volume command: {action}")
        ...
        try:
            if os.path.exists(self.console_command_file):
                ...
                if cmd:
                    ...
                    elif cmd.startswith("WII_VOLUME|"):
                        self.handle_wii_volume_command(cmd.split("|", 1)[1])
                    else:
    ...
    def refresh_input_screen_indicator(self):
        ...
        try:
            ...
            try:
                ...
                phone_mode = str(phone.get("mode", "laptop") or "laptop").lower()
                if phone_recent:
            ...
            if phone_recent and phone_mode in ("external", "viewer", "gsv"):
                self.set_input_screen_mode("external", phone_source)
            elif phone_recent:
    ...
    def finish_results_screen(self):
        ...
        # v28.26.12: after final results expires, the public/external display
        # must be the active target with the next game queued, even if the
        # operator had laptop control during gameplay.
        self.external_gsv_preferred = True
        self.set_input_screen_mode("external", "GSV")
        ...
        # mode can disagree.  The Wii helper gives two rumble pulses for EXTERNAL.
    ...
    def lights_should_run(self) -> bool:
        ...
        if self.host_state in (HostState.GAME_RUNNING, HostState.GAME_PAUSED):
            return False
    ...
    def cycle_allowed(self):
        return self.cycle_enabled.get() and self.lights_should_run() and self.host_state not in (HostState.GAME_RUNNING, HostState.GAME_PAUSED)

    def apply_attract_state(self):
        if self.all_lanes_test_active or self.host_state in (HostState.GAME_RUNNING, HostState.GAME_PAUSED, HostState.COUNTDOWN, HostState.GAME_SETUP):
            return
    ...
    def on_theme_brightness_changed(self, value):
        ...
        if self.host_state not in (HostState.GAME_RUNNING, HostState.GAME_PAUSED) and not self.all_lanes_test_active:
    ...
    def _candidate_controller_names(self) -> list:
        """Return best-effort controller names for choosing UI help cards."""
        names = []
        seen = set()
        try:
            for dev in getattr(self, "discovered_devices", []) or []:
        ...
        try:
            for pid in range(1, 5):
                name = str((self.controller_status.get(pid) or {}).get("name") or "").strip()
                if name and name not in seen:
                    names.append(name)
                    seen.add(name)
        except Exception:
    ...
    def _controller_help_profile_key(self) -> str:
        """Pick the most helpful controller-profile card for the current setup."""
        names = [n.lower() for n in self._candidate_controller_names()]
        for key, profile in CONTROLLER_HELP_CARD_PROFILES.items():
            tokens = tuple(str(t).lower() for t in profile.get("name_tokens", ()))
            if not tokens:
                continue
            if any(any(tok in name for tok in tokens) for name in names):
                return key

        # Fallback: if the configured labels use L/R shoulder names rather than
        # classic Xbox LB/RB naming, prefer the Switch-style help card because
        # that matches Dana's black-face-button controller best.
        try:
            ...
            left_label = str(labels.get("4", "")).strip().upper()
            right_label = str(labels.get("5", "")).strip().upper()
            if left_label == "L" and right_label == "R":
                return "switch_style"
        except Exception:
            pass
        return ""
    ...
    def _controller_help_profile(self) -> dict:
        key = self._controller_help_profile_key()
        if not key:
            return {}
        profile = dict(CONTROLLER_HELP_CARD_PROFILES.get(key) or {})
        profile["key"] = key
        return profile


    def _controller_help_image_path(self, kind: str) -> str:
        profile = self._controller_help_profile()
        if not profile:
            return ""
        filename = str(profile.get(f"{kind}_image") or "").strip()
        if not filename:
            return ""
    ...
    def _controller_join_hint(self) -> str:
        profile = self._controller_help_profile()
        if profile.get("join_hint"):
            return str(profile.get("join_hint"))
        return "Check-in opened. Arcade WHITE or Xbox L/Menu to join."


    def _show_checkin_help_card(self):
        image_path = self._controller_help_image_path("checkin")
        if image_path:
            self.viewer.show_checkin(image_path)
            if self.debug_logging.get():
                self.log(f"[HELP CARD] Check-in using {os.path.basename(image_path)}")
        else:
            self.viewer.show_checkin()


    def _show_select_colors_help_card(self, game_key: str = "") -> bool:
        """Show controller-specific color help when the selected game supports it."""
        cfg = getattr(self, "controller_actions", None) or copy.deepcopy(DEFAULT_CONTROLLER_ACTIONS)
        cfg = self._normalize_controller_actions_config(cfg)
        active_games = cfg.get("active_games", []) or []
        if game_key and game_key not in active_games:
            return False
        image_path = self._controller_help_image_path("select_colors")
        if image_path:
            self.viewer.show_select_colors(image_path)
            if self.debug_logging.get():
                self.log(f"[HELP CARD] Select-colors using {os.path.basename(image_path)}")
            return True
        return False
                ...
                if not self.button_map_mode and js.get_numaxes() >= 1:
                    try:
                        ...
                        if js.get_numaxes() >= 2:
                            ...
                            # Ascend needs held-stick state, not only edge transitions.
                            # Send the live Y-axis value every poll while running so a stick
                            # already held UP at leg start or after respawn immediately moves.
                            if self.current_game_key() == "ascend" and self.host_state == HostState.GAME_RUNNING and self.game_manager.is_running():
                                try:
                                    # Pygame reports UP as negative Y. Ascend expects UP as positive.
                                    self.game_manager.handle_input(player_id, f"P{player_id}_AXIS_Y", {"y": -float(y_axis), "continuous": True})
                                except Exception:
                                    pass
                                self.button_last_state[js_index][axis_y_key] = y_axis
                                continue
                            if self.current_game_key() == "ascend" and self.host_state != HostState.GAME_RUNNING:
                                self.button_last_state[js_index][axis_y_key] = y_axis
                                continue
                            
                            # Detect transition from center to up/down for non-Ascend games.
    ...
    def handle_button_press(self, player_id: int, color_name: str, pressed: bool = True):
        ...
        if self.host_state == HostState.CHECKIN_OPEN:
            ...
            return
        if self.host_state in (HostState.COUNTDOWN, HostState.GAME_PAUSED):
            # Ignore inputs during countdown or pause
            return
    ...
    # =========================================================================
    def refresh_start_pause_button(self):
        """Keep the header START button in sync with game state.

        START begins the selected game. During active gameplay it becomes
        PAUSE. While paused it becomes RESUME. STOP remains the hard abort /
        return-to-splash control.
        """
        if not hasattr(self, "start_pause_btn"):
            return
        try:
            if self.host_state == HostState.GAME_RUNNING:
                self.start_pause_btn.configure(text="PAUSE", bg="#d48a10", activebackground="#d48a10")
            elif self.host_state == HostState.GAME_PAUSED:
                self.start_pause_btn.configure(text="RESUME", bg="#2ea62e", activebackground="#2ea62e")
            else:
                self.start_pause_btn.configure(text="START", bg="#2ea62e", activebackground="#2ea62e")
        except Exception:
            pass

    def on_pause_game(self):
        """Soft-pause the active game without returning to the splash screen."""
        if self.host_state != HostState.GAME_RUNNING or not self.game_manager.is_running():
            return
        self.game_tick_active = False
        ...
        try:
            if pygame.mixer.get_init() and pygame.mixer.music.get_busy():
                ...
                self._music_paused_by_game_pause = True
        except Exception:
            pass
        self.set_state(HostState.GAME_PAUSED, "Game paused by operator")

    def on_resume_game(self):
        """Resume a soft-paused game."""
        if self.host_state != HostState.GAME_PAUSED or not self.game_manager.is_running():
            return
        try:
        ...
        try:
            if self._music_paused_by_game_pause and pygame.mixer.get_init():
                pygame.mixer.music.unpause()
        except Exception:
            pass
        self._music_paused_by_game_pause = False
        self.game_paused = False
        self.set_state(HostState.GAME_RUNNING, "Game resumed by operator")
        if not self.game_tick_active:
    ...
    def _center_toplevel_on_console(self, win, width: int, height: int):
        """Center a modal dialog over the laptop/console window."""
        try:
    ...
    def large_console_dialog(self, title: str, message: str, buttons=("OK",), default: str = None, danger: bool = False):
        """Show a large console modal for Wii/phone/trackball-friendly navigation.

        v28.26.17: The stock Tk messageboxes are too small for remote-style
        pointer control.  This helper creates a roughly 2x dialog with larger
        proportional buttons while preserving modal behavior.
        """
        result = {"value": None}
        default = default or (buttons[0] if buttons else "OK")
        width, height = 900, 430
        top = tk.Toplevel(self.root)
        top.title(title)
        top.configure(bg="#12061f")
        ...
        try:
            ...
            pass
        self._center_toplevel_on_console(top, width, height)

        border_color = "#ff6b4a" if danger else "#ffd84d"
        frame = tk.Frame(top, bg="#12061f", highlightbackground=border_color, highlightcolor=border_color, highlightthickness=5)
        ...
        tk.Label(
            ...
            font=("Arial", 34, "bold"),
            anchor="center",
        ).pack(fill="x", padx=28, pady=(26, 12))
        ...
        tk.Label(
            ...
            justify="center",
            wraplength=780,
        ).pack(fill="both", expand=True, padx=34, pady=(6, 18))

        btn_frame = tk.Frame(frame, bg="#12061f")
        ...
        for label in buttons:
            ...
            bg = "#1b63ff" if is_default else "#261035"
            if danger and label.upper() in ("YES", "RESET", "REDEEM / RESET"):
                bg = "#c93b1e"
            btn = tk.Button(
                btn_frame,
                text=label,
            ...
            btn.pack(side="left", expand=True, padx=22)
            if is_default:
                try:
                    btn.focus_set()
                except Exception:
        ...
        top.bind("<Return>", lambda e: choose(default))
        top.bind("<Escape>", lambda e: choose("NO" if "NO" in buttons else buttons[-1] if buttons else "OK"))
        try:
            top.grab_set()
            top.focus_force()
        except Exception:
    ...
    def large_warning_dialog(self, title: str, message: str):
        return self.large_console_dialog(title, message, buttons=("OK",), default="OK", danger=False)

    def large_redeem_reset_confirm(self) -> bool:
        choice = self.large_console_dialog(
            "Redeem / Reset",
            "Redeem tickets and clear the current player session?",
            buttons=("YES", "NO"),
            default="NO",
            danger=True,
        )
    ...
    def on_start_game(self):
        if self.host_state == HostState.GAME_RUNNING:
            self.on_pause_game()
            return
        if self.host_state == HostState.GAME_PAUSED:
            self.on_resume_game()
            return
        self.cancel_viewer_return()
        self.rescan_controllers()
        ...
        if self.players_joined.get() == 0:
            self.prepare_laptop_dialog_mouse("DIALOG")
            self.large_warning_dialog("No Players", "No players have joined.")
            return
        ...
        if not players:
            self.prepare_laptop_dialog_mouse("DIALOG")
            self.large_warning_dialog("No Players", "No players checked in.")
            return
        ...
        if requires_color_selection:
            ...
            if not self._show_select_colors_help_card(game_key):
    ...
    def on_stop_game(self):
        ...
        # If music was paused by PAUSE, pygame may report it as not busy, so force stop it.
        if getattr(self, "_music_paused_by_game_pause", False):
            try:
    ...
    def on_player_checkin(self):
        ...
        if self.host_state == HostState.CHECKIN_OPEN:
            ...
            if self.auto_enabled.get():
                ...
                self.update_auto_button()
            self.cancel_viewer_return()
            # v28.26.13: Check-In is a laptop/operator workflow.  If the operator
            # is already on the console and clicks Check-In, do not let the
            # external/GSV preference immediately steal focus back to monitor 2.
            self.external_gsv_preferred = False
            self.set_input_screen_mode("laptop", "CHECK-IN")
            self.force_wii_laptop_control("checkin_open")
            self.stop_music()  # Stop splash music during check-in
            self.attract.stop(self)
            self.falcon.clear_all_lanes(self)
            self.checkin_open = True
            self.players_confirmed = False
            self.set_state(HostState.CHECKIN_OPEN, self._controller_join_hint())
            self._show_checkin_help_card()  # Show check-in screen / controller help
    ...
    def on_redeem_points(self):
        self.prepare_laptop_dialog_mouse("DIALOG")
        if not self.large_redeem_reset_confirm():
            return
        self.cancel_viewer_return()
        ...
        # v28.26.17: A redeemed/reset session is a true public-home reset.  Clear
        # stale Wii Home return targets and previous game selection/audio so the
        # next A-toggle back to the external viewer stays on Pixel Challenge Home.
        self.selected_game.set("Splash")
        self.current_intro_index = -1
        self.final_results_active = False
        self.external_gsv_preferred = True
        self.wii_home_showing = False
        self.wii_home_return_active_id = ""
        self.wii_home_return_selected_game = ""
        try:
            self.viewer_show_splash()
        except Exception:
            pass
    ...
    def build_top_bar(self):
        ...
        self.start_pause_btn = self.neon_button(btns, "START", self.on_start_game, bg="#2ea62e", width=7)
    ...
    def _short_config_help(self, path_tuple, value):
        ...
        hints = {
            ...
            "min_spacing_px": "Minimum spawn distance between bands; runtime passing can still allow them to overlap later.",
            "allow_band_passing": "Allow faster bands to catch, overlap, and pass slower bands after spawning.",
    ...
    def _default_game_config_for_key(self, game_key):
        ...
        if game_key == "chomp_chase":
            return {
                "game_info": {"name": "Chomp Chase", "version": "1.0.1-easier"},
                "initial_lives": 4,
                "dot_spacing": 3,
                "player_move_ms": 110,
                "ghost_move_ms": 330,
                ...
                "fruit": {"enabled": True, "spawn_chance_per_sec": 0.035, "duration_sec": 7.0, "points": 500},
                "colors": {
                    "dot": [8, 8, 8],
                    "player": [255, 220, 0],
    ...
    def _validate_game_config_for_editor(self, game_key, config):
        ...
        elif game_key == "chomp_chase":
            clamp_num(cfg, "initial_lives", 1, 4, 4, True)
            clamp_num(cfg, "dot_spacing", 1, 20, 3, True)
            clamp_num(cfg, "player_move_ms", 30, 2000, 120, True)
            clamp_num(cfg, "ghost_move_ms", 30, 5000, 220, True)
            clamp_num(cfg, "scared_ghost_move_ms", 30, 5000, 260, True)
            clamp_num(cfg, "power_duration_sec", 0.5, 60.0, 6.0, False)
            clamp_num(cfg, "ghost_respawn_sec", 0.1, 10.0, 1.25, False)
            clamp_num(cfg, "timed_duration_sec", 10, 3600, 90, True)
            clamp_num(cfg, "objective_boards_to_clear", 1, 50, 3, True)
            fruit = cfg.get("fruit") if isinstance(cfg.get("fruit"), dict) else {}
            clamp_num(fruit, "spawn_chance_per_sec", 0.0, 1.0, 0.035, False)
            clamp_num(fruit, "duration_sec", 1.0, 60.0, 7.0, False)
            clamp_num(fruit, "points", 0, 100000, 500, True)
    ...
    def ensure_wii_ir_setup_vars(self):
        ...
        self.wii_minus_double_tap_var = tk.DoubleVar(value=self._safe_float_config(cfg, "wii_minus_double_tap_seconds", 0.38))
    ...
    def build_wii_ir_setup_section(self, parent):
        ...
        self._wii_ir_scale_row(inner, 8, "Minus double-tap", self.wii_minus_double_tap_var, 0.15, 0.80, 0.05,
                               "seconds allowed for mute")

        checks = tk.Frame(inner, bg="#1a1a2e")
        checks.grid(row=9, column=0, columnspan=4, sticky="w", pady=(4, 2))
        for text, var in (
        ...
        bounds.grid(row=10, column=0, columnspan=4, sticky="w", pady=(6, 2))
        ...
        calib.grid(row=11, column=0, columnspan=4, sticky="we", pady=(8, 4))
        ...
        tk.Label(inner, textvariable=self.wii_ir_diag_status_var, bg="#1a1a2e", fg="#c0ffc0",
                 font=("Arial", 8, "italic"), wraplength=1000, justify="left").grid(row=12, column=0, columnspan=4, sticky="we", pady=(2, 0))

        buttons = tk.Frame(inner, bg="#1a1a2e")
        buttons.grid(row=13, column=0, columnspan=4, sticky="w", pady=(8, 2))
        ...
        tk.Label(inner, textvariable=self.wii_ir_setup_status_var, bg="#1a1a2e", fg="#8ec5ff",
                 font=("Arial", 9, "italic")).grid(row=14, column=0, columnspan=4, sticky="w", pady=(3, 0))
        tk.Label(inner, text="Absolute mode maps calibrated raw IR midpoint directly into the console screen. Live Apply writes wii_menu_wand_config.json; no wand restart needed.",
                 bg="#1a1a2e", fg="#888888", font=("Arial", 8, "italic")).grid(row=15, column=0, columnspan=4, sticky="w", pady=(2, 0))
    ...
    def apply_wii_ir_mouse_setup(self, show_popup=False):
        ...
        cfg.update({
            ...
            "wii_minus_double_tap_seconds": round(float(self.wii_minus_double_tap_var.get()), 3),
        })
        notes = cfg.get("notes")
        ...
        notes["WII_MINUS_DOUBLE_TAP_SETUP"] = "Minus double-tap controls the maximum time between two Wii - taps for muting. A double-tap mute no longer applies a volume-down step before muting."
        cfg["notes"] = notes
        try:
            tmp = WII_MENU_WAND_CONFIG_FILE + ".tmp"
            with open(tmp, "w", encoding="utf-8") as f:
    ...
    def open_setup_window(self):
        ...
        if self.setup_geometry:
            ...
            self.setup_window.geometry("1200x850+180+100")
